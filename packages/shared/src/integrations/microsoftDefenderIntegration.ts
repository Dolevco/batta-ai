...
import { Tool, ToolResult, ToolCategory } from '@ai-agent/core';
import { CustomIntegrationHandler } from '../types';

export const MsDefenderCategory: ToolCategory = {
  name: 'microsoft-defender',
  description: 'Microsoft Defender for Cloud (MDC) security tools',
...
export interface MDCSecurityAssessment {
  id: string;
  resourceId: string;
  displayName: string;
  status: string;
...
export interface MDCSecuritySubAssessment {
  id: string;
  resourceId: string;
  displayName: string;
  status: string;
