/**
 * FeatureService — read/query BusinessFeature entities from the data store.
...
function normalizeDfd(dfd: DataFlowDiagram): DataFlowDiagram {
  return {
    ...dfd,
...
export class FeatureService {
  ...
  async getFeaturesByTenant(tenantId: TenantId): Promise<BusinessFeatureSummary[]> {
    const entities = await this.dataAdapter.listEntities(tenantId, 'feature_analysis', 500);
    return entities
  ...
  async getFeatureById(
    ...
    const entity = await this.dataAdapter.getEntity(tenantId, id);
    ...
    const feature = raw.metadata as unknown as BusinessFeature;
    // Older records may be missing threatModel or dataFlowDiagram sub-fields
    if (!feature.threatModel) {
      feature.threatModel = {
        strideThreats: [],
      ...
      if (!tm.strideThreats) tm.strideThreats = [];
      if (!tm.trustBoundaryAnalysis) tm.trustBoundaryAnalysis = [];
      if (!tm.dataClassificationSummary) tm.dataClassificationSummary = [];
      if (!tm.complianceConsiderations) tm.complianceConsiderations = [];
      if (!tm.attackVectors) tm.attackVectors = [];
      if (!tm.securityRecommendations) tm.securityRecommendations = [];
      if (tm.overallRiskScore == null) tm.overallRiskScore = 0;
    }
    if (!feature.dataFlowDiagram) {
      feature.dataFlowDiagram = { actors: [], processes: [], dataStores: [], flows: [], trustBoundaries: [] };
    } else {
      feature.dataFlowDiagram = normalizeDfd(feature.dataFlowDiagram);
    }
    return feature;
  }
   ...
   * Results are scoped to the tenant via the the data store documentType+tenantId filter.
   */
  async searchFeaturesSemantic(
    ...
    const semanticResults = await this.dataAdapter.searchSemanticDocumentsByType(
      tenantId,
      query,
      'feature',
      limit
   ...
   * name terms.  Both scoped names (e.g. "@batta/api") and bare names
   * (e.g. "api") are matched exactly in the DB via the data store's `match.any` filter
   * — no in-memory scan of all services.
   *
   * If `repositoryId` is provided it is added as an additional the data store filter to
   ...
   * values are passed as a structured array/value to the the data store client library,
  ...
  async getCodeServicesByNames(
    ...
    const entities = await this.dataAdapter.listEntitiesByNames(tenantId, 'code_service', terms, 50, repoId || undefined);
   ...
   * the data store filter value, never interpolated into any query string.
   */
  async resolveRepositoryIdByName(
    ...
    const entities = await this.dataAdapter.listEntitiesByNames(tenantId, 'code_repository', [name], 1);
