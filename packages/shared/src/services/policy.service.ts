...
import {
  ...
  WORK_ITEM_QUESTIONS,
  WORK_ITEM_TASK_RULES,
  WORK_ITEM_BASELINE_TASKS,
} from './security-review-defaults';
...
> = {
  ...
  work_item_review: {
    ...
    description: 'Security gate for Jira stories, tasks, and bugs before delivery.',
    questions: WORK_ITEM_QUESTIONS,
    taskRules: WORK_ITEM_TASK_RULES,
    baselineTasks: WORK_ITEM_BASELINE_TASKS,
  },
};
...
export class PolicyService {
  ...
  async getAll(tenantId: string): Promise<PolicyTemplate[]> {
    // Always seed to ensure any newly-added policy types are created for existing tenants
  ...
  async seedDefaultPolicies(tenantId: string): Promise<PolicyTemplate[]> {
    ...
    const types: PolicyTemplateType[] = ['security_review', 'responsible_ai', 'privacy', 'work_item_review'];
