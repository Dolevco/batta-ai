...
import { BASE_QUESTIONS, TASK_RULES, BASELINE_TASKS, WORK_ITEM_QUESTIONS, WORK_ITEM_TASK_RULES, WORK_ITEM_BASELINE_TASKS } from './security-review-defaults';
import type { JiraWorkItemContext } from '../types';
import type { SecurityReviewJiraExportService } from './security-review-jira-export.service';
...
export class SecurityReviewService {
  constructor(
    ...
    private jiraExportService?: SecurityReviewJiraExportService,
  ) {
  }
  ...
  async startWorkItemReview(
    ...
    let snapshotTaskRules: PolicyTaskRule[] = WORK_ITEM_TASK_RULES;
    let snapshotBaselineTasks: Omit<SecurityTask, 'id'>[] = WORK_ITEM_BASELINE_TASKS;
    let policyTemplateVersion: number | undefined;
    let policyQuestions: SecurityReviewQuestion[] = WORK_ITEM_QUESTIONS;

    if (this.policyRepository) {
      try {
        const activePolicy = await this.policyRepository.getActiveByType(tenantId, 'work_item_review');
        if (activePolicy) {
          policyQuestions = activePolicy.questions;
          snapshotTaskRules = activePolicy.taskRules;
          snapshotBaselineTasks = activePolicy.baselineTasks;
    ...
    const review: SecurityReview = {
      ...
      ...(options.humanResponsible !== undefined && { humanResponsible: options.humanResponsible }),
      status: 'questionnaire_pending',
      source: 'jira_work_item',
      jiraWorkItemContext: options.jiraWorkItemContext,
      agentStatus: 'pending',
      ...
      snapshotBaselineTasks,
      ...(policyTemplateVersion !== undefined && { policyTemplateVersion }),
      policyTemplateType: 'work_item_review',
      createdAt: now,
      updatedAt: now,
    };
  ...
  async submitAttestations(
    ...
    this.validateAttestations(attestations, review.tasks);
    ...
    // Phase 4: auto-export tasks to Jira when the policy's jiraActionItems.autoCreate is set.
    if (this.jiraExportService && this.policyRepository) {
      setImmediate(() => this.tryAutoExportToJira(tenantId, id, attested).catch(err => {
        console.error(`[SecurityReviewService] auto-export failed for review ${id}:`, err?.message ?? err);
      }));
    }
  ...
  private async tryAutoExportToJira(tenantId: string, reviewId: string, review: SecurityReview): Promise<void> {
    if (!this.jiraExportService || !this.policyRepository) return;
    const policyType = review.policyTemplateType ?? 'security_review';
    const policy = await this.policyRepository.getActiveByType(tenantId, policyType);
    const cfg = policy?.jiraActionItems;
    if (!cfg?.autoCreate || !cfg.targetProjectKey) return;

    await this.jiraExportService.exportTasks(tenantId, reviewId, {
      severityThreshold: cfg.severityThreshold,
      targetProjectKey: cfg.targetProjectKey,
      issueType: cfg.issueType || 'Task',
      priorityMap: cfg.priorityMap ?? { critical: 'Highest', high: 'High', medium: 'Medium', low: 'Low' },
      onlyUnhandled: true,
    });
  }
  ...
  async explainRequirement(id: string, tenantId: string, taskId: string): Promise<SecurityRequirementExplanation> {
    const review = await this.requireReview(id, tenantId);
    const task = review.tasks.find(t => t.id === taskId);
    if (!task) throw new Error(`Security task not found: ${taskId}`);

    const rules = review.snapshotTaskRules ?? TASK_RULES;
    const baselineTasks = review.snapshotBaselineTasks ?? BASELINE_TASKS;
    const rule = rules.find(candidate =>
      candidate.tasks.some(ruleTask => this.sameTask(ruleTask, task)),
    );
    const baselineMatch = baselineTasks.some(baselineTask => this.sameTask(baselineTask, task));
    const triggeringAnswers = rule
      ? review.answers.filter(answer => answer.questionId === rule.questionId)
      : [];
    const question = rule
      ? review.questions.find(q => q.id === rule.questionId)
      : undefined;

    const reason = rule
      ? `This task was generated because the answer to "${question?.question ?? rule.questionId}" indicated the change touches ${task.principle.toLowerCase()} concerns. The policy snapshot maps that answer to this requirement.`
      : baselineMatch
  ...
  private validateAttestations(attestations: SecurityAttestation[], tasks: SecurityTask[]): void {
    ...
    attestations.forEach((attestation, index) => {
      ...
      if (this.containsSecretLikeContent(notes)) {
      ...
      if (!this.hasEvidenceLikeDetail(notes)) {
        throw new Error(
  ...
  private hasEvidenceLikeDetail(value: string): boolean {
    const patterns = [
      /\b[\w.-]+\/[\w./-]+\.[A-Za-z0-9]+\b/,
  ...
  private sameTask(a: Omit<SecurityTask, 'id'>, b: SecurityTask): boolean {
