...
// ── Work Item Review defaults ─────────────────────────────────────────────────

export const WORK_ITEM_QUESTIONS: SecurityReviewQuestion[] = [
  {
    id: 'wi_data_handling',
    question: 'Does this work item involve creating, reading, updating, or deleting personal or sensitive data?',
    hint: 'Include PII, financial records, credentials, or any data classified above "public".',
  },
  {
    id: 'wi_new_endpoints',
    question: 'Does this work item introduce or modify API endpoints, webhooks, or event consumers?',
    hint: 'Include REST routes, GraphQL mutations/queries, Kafka consumers, and webhook receivers.',
  },
  {
    id: 'wi_auth_change',
    question: 'Does this work item change authentication, authorization, or access control logic?',
    hint: 'Include permission checks, role assignments, token flows, or session management.',
  },
  {
    id: 'wi_external_integration',
    question: 'Does this work item integrate with an external service, vendor API, or third-party SDK?',
    hint: 'Include payment processors, email providers, analytics SDKs, cloud services, or any SaaS.',
  },
  {
    id: 'wi_data_migration',
    question: 'Does this work item require a database migration, schema change, or data backfill?',
    hint: 'Include adding/removing columns, index changes, data type changes, or large-scale data moves.',
  },
  {
    id: 'wi_infra_change',
    question: 'Does this work item change infrastructure, deployment configuration, or CI/CD pipelines?',
    hint: 'Include Dockerfile changes, Terraform/Bicep, environment variables, secrets, or pipeline YAML.',
  },
];

export const WORK_ITEM_TASK_RULES: PolicyTaskRule[] = [
  {
    questionId: 'wi_data_handling',
    tasks: [
      {
        title: 'Confirm data classification and access controls for all affected data stores',
        description:
          'Identify every data store touched by this work item. Verify each has the correct classification tag ' +
          'and that only the minimum required service accounts have write access.',
        severity: 'high',
        principle: 'Data Governance',
      },
      {
        title: 'Verify encryption at rest and in transit for sensitive data paths',
        description:
          'Confirm TLS 1.2+ on all transit paths and AES-256 (or equivalent) at rest. ' +
          'Ensure PII is never written to logs.',
        severity: 'critical',
        principle: 'Data Protection',
      },
    ],
  ...
  {
    questionId: 'wi_new_endpoints',
    tasks: [
      {
        title: 'Apply authentication and authorization checks on all new endpoints',
        description:
          'Deny by default. Every new route must have an explicit auth check. ' +
          'Document the expected caller identity and permission scope.',
        severity: 'critical',
        principle: 'Least Privilege',
      },
      {
        title: 'Apply rate limiting and input validation on all new endpoints',
        description:
          'Set per-IP/token rate limits. Validate all input with an allow-list schema. ' +
  ...
  {
    questionId: 'wi_auth_change',
    tasks: [
      {
        title: 'Enforce least-privilege on all modified access control paths',
        description:
          'Review every changed permission check. Ensure role escalation requires explicit approval and ' +
          'that no implicit grant is introduced.',
        severity: 'critical',
        principle: 'Least Privilege',
      },
    ],
  ...
  {
    questionId: 'wi_external_integration',
    tasks: [
      {
        title: 'Assess new dependency for known vulnerabilities and license compliance',
        description:
          'Run npm audit / pip-audit / equivalent. Check CVE history, maintenance status, and license. ' +
          'Pin exact version in lock file.',
        severity: 'high',
        principle: 'Supply Chain Security',
      },
      {
        title: 'Store third-party credentials in the secrets manager',
        description:
          'API keys, tokens, and connection strings for the new integration must be stored in Key Vault / ' +
          'Secrets Manager — never in code, config files, or environment variable defaults committed to git.',
        severity: 'critical',
        principle: 'Secrets Management',
      },
    ],
  ...
  {
    questionId: 'wi_data_migration',
    tasks: [
      {
        title: 'Test migration with a production data snapshot before rollout',
        description:
          'Run the migration against an anonymized copy of production data. ' +
          'Verify rollback script exists and has been tested. Document expected downtime or locking duration.',
        severity: 'high',
        principle: 'Resilience',
      },
    ],
  ...
  {
    questionId: 'wi_infra_change',
    tasks: [
      {
        title: 'Review CI/CD changes for secret exposure and privilege escalation',
        description:
          'Pipeline steps must not print secrets to logs, must not grant overly broad cloud permissions, ' +
          'and must not be triggerable by untrusted actors (e.g. fork PRs).',
        severity: 'critical',
        principle: 'Supply Chain Security',
      },
    ],
...
export const WORK_ITEM_BASELINE_TASKS: Omit<SecurityTask, 'id'>[] = [
  {
    title: 'Link this security review to the Jira work item in the attestation',
    description:
      'After implementation, record the Jira issue key in the attestation notes and confirm the work item ' +
      'acceptance criteria explicitly includes security task completion.',
    severity: 'medium',
    principle: 'Accountability',
  },
];
