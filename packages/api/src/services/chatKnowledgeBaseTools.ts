 ...
 *     Filtering is done server-side — the LLM never receives a full dump to filter itself.
 *
 * Security controls:
 ...
 *  - All errors are caught by safeGraph()/safeTable(); only generic messages reach the caller.
 *  - Read-only: no writes to any store.
 *  - Tenant isolation enforced by DataAdapter (tenantId injected server-side).
 */
...
import type { GraphProjection, TableProjection, TableColumn } from '@batta/core';
import type { FeatureService } from '@batta/shared';
import type { SecurityReviewService } from '@batta/shared';
...
const KnowledgeBaseCategory: ToolCategory = {
  name: 'knowledge_base',
 ...
 */
class GetFeatureGraphTool extends GraphBaseTool<GetFeatureGraphParams> {
 ...
 */
class SemanticSearchServicesTool extends TableBaseTool<SearchServicesParams> {
  ...
  async execute(p: SearchServicesParams): Promise<TableToolResult> {
    try {
      ...
      const results = await this.dataAdapter.searchSemanticDocumentsByType(
        this.tenantId,
        query,
        'service',
        limit
 ...
 */
class SemanticSearchFeaturesTool extends TableBaseTool<SearchFeaturesParams> {
 ...
 * All filtering is performed server-side (database payload filters) or in this
 ...
 */
class FilterSecurityReviewsTool extends TableBaseTool<FilterReviewsParams> {
...
export function createKnowledgeBaseChatTools(
  ...
  return [
    ...
    new SemanticSearchServicesTool(dataAdapter, tenantId),
