 ...
 * service-to-service relationships stored in the graph database by the data-indexer:
 *
 ...
 *   - Graph queries use the GraphAdapter methods which accept only typed IDs and
 *     relationship types — no user input is interpolated into query strings.
 *   - Entity lookups go through the DataAdapter (listEntities / getEntity) which apply
...
import type { GraphProjection, GraphNode, GraphEdge } from '@batta/core';
import type { PostgresGraphAdapter as GraphAdapter } from '@batta/shared';
import type { PostgresDataAdapter as DataAdapter } from '@batta/shared';
import type { CanonicalEntity } from '@batta/shared';
...
const ServiceRelationshipCategory: ToolCategory = {
 ...
 *
 * Security: uses DataAdapter.listEntities which always scopes to tenantId.
...
async function findServiceByName(
  ...
  const services = await dataAdapter.listEntities(tenantId, 'code_service', 500);
 ...
 */
class GetServiceCallersTool extends GraphBaseTool<ServiceNameOrIdParams> {
  ...
  async execute(p: ServiceNameOrIdParams): Promise<GraphToolResult> {
    return safeGraph(async () => {
      ...
      if (idRaw) {
        target = await this.dataAdapter.getEntity(this.tenantId, idRaw);
      } else if (nameRaw) {
        target = await findServiceByName(this.dataAdapter, this.tenantId, nameRaw);
      }
      ...
      const callerRels = await this.graphAdapter.getRelationshipsByTarget(
      ...
      const callerEntities = await Promise.all(
        callerRels.map(r => this.dataAdapter.getEntity(this.tenantId, r.sourceId)),
      );
 ...
 */
class GetServiceDependenciesTool extends GraphBaseTool<ServiceNameOrIdParams> {
  ...
  async execute(p: ServiceNameOrIdParams): Promise<GraphToolResult> {
    return safeGraph(async () => {
      ...
      if (idRaw) {
        source = await this.dataAdapter.getEntity(this.tenantId, idRaw);
      } else if (nameRaw) {
        source = await findServiceByName(this.dataAdapter, this.tenantId, nameRaw);
      }
      ...
      const depRels = await this.graphAdapter.getRelationshipsBySource(
      ...
      const depEntities = await Promise.all(
        depRels.map(r => this.dataAdapter.getEntity(this.tenantId, r.targetId)),
      );
 ...
 */
class GetServiceApiRoutesTool extends BaseTool<ServiceNameOrIdParams> {
  ...
  async execute(p: ServiceNameOrIdParams): Promise<ToolResult> {
    return safe(async () => {
      ...
      const apiRels = await this.graphAdapter.getRelationshipsByTarget(
      ...
      await Promise.all(apiRels.map(async rel => {
        ...
        const callerEntity = await this.dataAdapter.getEntity(this.tenantId, rel.sourceId);
 ...
 */
class GetServiceCallGraphTool extends GraphBaseTool<ServiceNameOrIdParams> {
  ...
  async execute(p: ServiceNameOrIdParams): Promise<GraphToolResult> {
    return safeGraph(async () => {
      ...
      if (idRaw) {
        focus = await this.dataAdapter.getEntity(this.tenantId, idRaw);
      } else if (nameRaw) {
        focus = await findServiceByName(this.dataAdapter, this.tenantId, nameRaw);
      }
      ...
      const [callerRels, calleeRels] = await Promise.all([
        this.graphAdapter.getRelationshipsByTarget(this.tenantId, focus.id, 'CALLS_SERVICE' as any),
        this.graphAdapter.getRelationshipsBySource(this.tenantId, focus.id, 'CALLS_SERVICE' as any),
      ]);
      ...
      await Promise.all(
        Array.from(peerIds).map(async id => {
          const e = await this.dataAdapter.getEntity(this.tenantId, id);
...
export function createServiceRelationshipTools(
  ...
  return [
    new GetServiceCallersTool(graphAdapter, dataAdapter, tenantId),
    new GetServiceDependenciesTool(graphAdapter, dataAdapter, tenantId),
    new GetServiceApiRoutesTool(graphAdapter, dataAdapter, tenantId),
    new GetServiceCallGraphTool(graphAdapter, dataAdapter, tenantId),
  ];
}
