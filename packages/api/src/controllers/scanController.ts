...
export class ScanController {
  ...
  async listRepositories(req: Request, res: Response): Promise<void> {
    try {
      ...
      const repos = await this.scanOrchestrator.discoverRepositories(tenantId);
      res.json(repos);
    } catch (err) {
  ...
  async streamScan(req: Request, res: Response): Promise<void> {
    try {
      ...
      const missingCapability = await this.getMissingPortalScanCapability(tenantId, options);
      if (missingCapability) {
        res.status(422).json({
          error: 'Portal scan requires additional setup.',
      ...
      try {
        await this.scanOrchestrator.streamScan(tenantId, options, sendEvent);
      } finally {
  ...
  async triggerScan(req: Request, res: Response): Promise<void> {
    try {
      ...
      const { scanId, alreadyRunning } = await this.scanOrchestrator.startScan(tenantId, options);
  ...
  async getScanStatus(req: Request, res: Response): Promise<void> {
    try {
      ...
      const scan = this.scanOrchestrator.getScan(tenantId, scanId);
      if (!scan) {
  ...
  async listScanHistory(req: Request, res: Response): Promise<void> {
    try {
      const tenantId = req.auth!.tenantId;
      const scans = this.scanOrchestrator.listScans(tenantId);
      res.json(scans);
    } catch (err) {
  ...
  private async getMissingPortalScanCapability(tenantId: string, options: ScanOptions) {
    const capabilities = await this.capabilityService.getCapabilities(tenantId);
    const capabilityId = options.scope === 'cloud' ? 'portalCloudScan' : 'portalCodeScan';
    const capability = capabilities.capabilities.find(item => item.id === capabilityId);
    return capability?.available ? undefined : capability;
  }
}
