...
import type { PolicyService } from '@ai-agent/shared';
import type { PolicyTemplateType } from '@ai-agent/shared';
...
const VALID_TYPES: PolicyTemplateType[] = ['security_review', 'responsible_ai', 'privacy'];
...
export class PolicyController {
  ...
  async updatePolicy(req: Request, res: Response): Promise<void> {
    try {
      const tenantId = resolveTenantId(req);
      const { id } = req.params;
      const { questions, taskRules, baselineTasks, isActive, name, description } = req.body;
