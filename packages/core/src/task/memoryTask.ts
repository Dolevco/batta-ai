 ...
 */
export interface MemoryTaskConfig<TOutput = unknown> {
...
export class MemoryTask<TInput = unknown, TOutput = unknown> {
  ...
  constructor(
    api: ILLMApiHandler,
    config: MemoryTaskConfig<TOutput>
  ) {
   ...
   */
  private buildCustomInstructions(config: MemoryTaskConfig<TOutput>): string {
