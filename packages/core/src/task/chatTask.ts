...
import { ILLMApiHandler, IEmbeddingHandler, StreamChunk } from '../llm';
...
export class ChatTask extends Task {
  ...
  protected getTaskCompletionResult<T>(toolResult: ToolResult, tool: Tool | undefined): TaskResult<T> {
    ...
    if (tool?.name === ChatCompletionToolName) {
      const r = toolResult.result as Record<string, unknown> | undefined;
      // If includeGraph requested, attach the latest graph
      if (r?.includeGraph && this.latestGraphResult?.graph) {
        return {
          ...baseResult,
          result: { ...r, graph: this.latestGraphResult.graph } as T,
        };
      }
      // If includeTable requested, attach the latest table (mutually exclusive with graph)
      if (r?.includeTable && this.latestTableResult?.table) {
        return {
          ...baseResult,
          result: { ...r, table: this.latestTableResult.table } as T,
        };
      }
    }
