...
import { ILLMApiHandler, IEmbeddingHandler, CompletionResponse } from '../llm';
...
export class Task {
  ...
  constructor(api: ILLMApiHandler, config: TaskConfig, embeddingHandler?: IEmbeddingHandler) {
    ...
    if (enableLongTermMemory && embeddingHandler && effectiveConfig.longTermMemory?.tenantId) {
  ...
  protected getTaskCompletionResult<T>(toolResult: ToolResult, _tool: Tool | undefined): TaskResult<T> {
    return {
      success: toolResult.success,
      completed: true,
      summary: toolResult.message,
      result: toolResult.result as T | undefined,
      ...
      if (parsedTurn.toolUses.length > 0) {
        ...
        for (let i = 0; i < parsedTurn.toolUses.length; i++) {
          ...
          if (!!tool?.isInteractionTool) {
            return { success: result.success, completed: false, summary: result.result as string, result: result.result as T };
          }
        }
