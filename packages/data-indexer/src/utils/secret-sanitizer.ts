...
/**
 * Keywords that indicate secret-related content
 */
 ...
 */
export function sanitizeSecrets(text: string | undefined | null): string {
  if (!text) {
    return '';
  }
 ...
 * @returns True if text likely contains secrets
 */
export function containsSecrets(text: string | undefined | null): boolean {
  if (!text) {
    return false;
  }
  ...
  const hasKeyword = SECRET_KEYWORDS.some(keyword => lowerText.includes(keyword));
  
  if (!hasKeyword) {
    return false;
  }

  // If has keyword, check if it matches any pattern
  for (const pattern of SECRET_PATTERNS) {
...
/**
 * Sanitize evidence object by removing secrets from reason and evidence fields
 * 
 * @param evidence - Evidence object with potential secrets
 * @returns Sanitized evidence object
 */
export function sanitizeEvidence(evidence: {
  ...
  if (sanitized.reason) {
    sanitized.reason = sanitizeSecrets(sanitized.reason);
  }

  if (sanitized.evidence) {
    if (Array.isArray(sanitized.evidence)) {
      sanitized.evidence = sanitized.evidence.map(e => sanitizeSecrets(e));
    } else {
      sanitized.evidence = sanitizeSecrets(sanitized.evidence);
    }
  }
