  ...
  async transform(
    ...
    for (const service of extraction.services) {
      try {
        ...
        // repo→service CONTAINS edges are intentionally omitted from the graph
      } catch (error: any) {
        errors.push({
          stage: 'transformation',
    ...
    for (const build of extraction.buildArtifacts) {
      try {
        ...
        // repo→build artifact CONTAINS edges are intentionally omitted from the graph
    ...
    for (const deployment of extraction.deploymentArtifacts) {
      try {
        ...
        // repo→deployment artifact CONTAINS edges are intentionally omitted from the graph
