 ...
 *
 *   Stage 1 – EXTRACT_TRANSFORM       Extract code entities, transform, persist to data + graph stores
 ...
 *   Stage 5.5 – DATA_STORE_CONSOLIDATION Promote DFD/static evidence into DataStore entities
...
import { TenantId, CanonicalEntity, Relationship, Evidence, SemanticDocument } from '@batta/shared';
import type { IIndexingRunRepository } from '@batta/shared';
import type { IndexingRun } from '@batta/shared';
import {
  IndexRepositoryTask,
  IndexingDomain,
  TaskCheckpoint,
  TaskStage,
  TaskResult,
} from './indexing-task.types';
import { EntityIdUtils } from '../utils/id-generator';
import { CodeExtractionStage } from './stages/code-extraction.stage';
import { CodeTransformationStage } from './stages/code-transformation.stage';
import { CodePersistenceStage } from './stages/code-persistence.stage';
import { DataStoreConsolidationStage } from './stages/data-store-consolidation.stage';
import { CloudDiscoveryStage } from './stages/cloud-discovery.stage';
import { BusinessFeatureExtractor } from '../analysis/business-feature-extractor';
import { ServiceRelationshipsExtractor } from '../analysis/service-relationships';
import { RepositoryResponsibilityCalculator } from '../analysis/repository-responsibility-calculator';
import { RepositoryBriefingService } from '../analysis/repository-briefing';
import { CheckpointManager } from './checkpoint-manager';
import { IntegrationFetcher } from '../integrations/integration-fetcher';
import { RepositorySetup } from '../integrations/repository-setup';
import { CloudResourceRepository } from '../cloud/repository/cloud-resource-repository';
import type { CodeIndexerConfig } from './stages/code-discovery.stage';
import type { CodeService, DeploymentArtifact, BuildArtifact, RepositoryBriefing } from '@batta/shared';
import { createDataIndexerRegistry } from '../agents';
import { cloudGraphToEntities } from '../cloud/providers/azure/cloud-graph-to-entities';
...
export class RepositoryTaskProcessor {
  ...
  private dataStoreConsolidationStage: DataStoreConsolidationStage;
  ...
  constructor(tenantId: TenantId, config: TaskProcessorConfig) {
    ...
    this.persistenceStage = new CodePersistenceStage(config.dataAdapter, config.graphAdapter);
    ...
    this.serviceRelationshipsExtractor = new ServiceRelationshipsExtractor(registry, config.graphAdapter, config.dataAdapter);
    this.repositoryResponsibilityCalculator = new RepositoryResponsibilityCalculator(config.api, config.dataAdapter, config.graphAdapter);
    this.repositoryBriefingService = new RepositoryBriefingService(registry, config.dataAdapter, config.graphAdapter);
    this.businessFeatureExtractor = new BusinessFeatureExtractor(registry, config.dataAdapter, config.graphAdapter);
    this.dataStoreConsolidationStage = new DataStoreConsolidationStage();
  ...
  async processTask(
    ...
    try {
      ...
      // ── Stage 5.5: Data Store Consolidation ─────────────────────────────
      // Reads code_service + feature_analysis entities from the data store (both are now persisted)
      // and promotes DFD/static-analysis evidence into first-class DataStore entities.
      await this.runDataStoreConsolidationStage(task, result, onStageChange);
      ...
      //   - cloud relationships (in graph store)
      if (runFeatures) {
  ...
  private async runExtractTransformStage(
    ...
    // Must run after persistEntities so the graph store can find the source/target nodes.
  ...
  private async runCloudDiscoveryStage(
    ...
    if (!cloudDiscovery.cloudGraph) {
    ...
    } else if (!this.config.graphAdapter) {
      console.warn(`[${task.taskId}] ⚠️  Cloud graph produced but graph adapter is not configured — graph topology will not be persisted.`);
    } else {
      ...
      try {
        await this.config.graphAdapter.storeCloudGraph(cloudDiscovery.cloudGraph);

        // Write cloud graph topology nodes to the data store so the SecurityQueryTools BFS
        // can traverse them. Uses the same 16-char hex IDs stored in the graph store.
        // Classification: CONFIDENTIAL — infrastructure topology; tenantId-isolated.
        const cloudGraphEntities = cloudGraphToEntities(cloudDiscovery.cloudGraph);
        if (cloudGraphEntities.length > 0) {
          console.log(`[${task.taskId}] 💾 Persisting ${cloudGraphEntities.length} cloud graph topology nodes...`);
          await this.config.dataAdapter.storeEntities(cloudGraphEntities);
        }
  ...
  private async runLLMCorrelationStage(
    ...
    if (resolvedRunType === 'incremental') {
      ...
      // from the graph and data stores directly.
  ...
  private async runFeatureExtractionStage(
    ...
    for (const service of services) {
      try {
        ...
        // Re-read the service from the data store — Step 5 persisted serviceDfd + serviceThreatModel
        // (and the projected threatModel) onto it. Stage 6 needs the latest version.
        const updatedService = await this.config.dataAdapter?.getEntity(task.tenantId, service.id);
        if (updatedService) {
  ...
  private async runDataStoreConsolidationStage(
    task: IndexRepositoryTask,
    result: TaskResult,
    onStageChange?: (stage: TaskStage, status: 'running' | 'completed' | 'failed', itemsProcessed?: number) => void,
  ): Promise<void> {
    const stageStart = Date.now();
    onStageChange?.(TaskStage.DATA_STORE_CONSOLIDATION, 'running');

    try {
      if (!this.config.dataAdapter) {
        console.log(`[${task.taskId}] Data store consolidation skipped — no data adapter`);
        onStageChange?.(TaskStage.DATA_STORE_CONSOLIDATION, 'completed', 0);
        return;
      }

      // Delete all existing data_store entities before re-consolidating.
      // Consolidation is a full recompute — IDs are deterministic so upserts would be
      // idempotent for same-run results, but old entities from prior clustering strategies
      // (different key → different ID) must be purged from both data and graph stores.
      await Promise.all([
        this.config.dataAdapter.deleteEntitiesByType(task.tenantId, 'data_store'),
        this.config.graphAdapter?.deleteEntitiesByType(task.tenantId, 'data_store'),
      ]);

      // Read all persisted service + feature entities for this tenant.
      // These were written by Stage 1 and Stage 5 respectively.
      const [services, features, cloudResources] = await Promise.all([
        this.config.dataAdapter.listEntities(task.tenantId, 'code_service', 1000),
        this.config.dataAdapter.listEntities(task.tenantId, 'feature_analysis', 1000),
        this.config.dataAdapter.listEntities(task.tenantId, 'cloud_resource', 1000),
      ]);

      console.log(`[${task.taskId}] Data Store Consolidation: ${services.length} services, ${features.length} features, ${cloudResources.length} cloud resources`);

      const allEntities = [...services, ...features, ...cloudResources];
      const dsResult = this.dataStoreConsolidationStage.run(task.tenantId, allEntities);

      if (dsResult.dataStores.length > 0) {
        await this.persistenceStage.persistEntities(task.tenantId, dsResult.dataStores, []);
        if (dsResult.relationships.length > 0) {
          await this.persistenceStage.persistRelationships(task.tenantId, dsResult.relationships);
        }
        console.log(`[${task.taskId}] Data Store Consolidation: persisted ${dsResult.dataStores.length} stores, ${dsResult.relationships.length} relationships`);
      } else {
        console.log(`[${task.taskId}] Data Store Consolidation: no stores found`);
      }

      result.summary.errors.push(...dsResult.errors.map(e => e.message));
      result.stages[TaskStage.DATA_STORE_CONSOLIDATION] = {
        completed: true,
        duration: Date.now() - stageStart,
        itemsProcessed: dsResult.dataStores.length,
      };
      onStageChange?.(TaskStage.DATA_STORE_CONSOLIDATION, 'completed', dsResult.dataStores.length);
    } catch (err: any) {
      console.error(`[${task.taskId}] Data store consolidation failed:`, err?.message ?? err);
      result.summary.errors.push(`Data store consolidation error: ${err?.message ?? String(err)}`);
      result.stages[TaskStage.DATA_STORE_CONSOLIDATION] = {
        completed: false,
        duration: Date.now() - stageStart,
        itemsProcessed: 0,
      };
      onStageChange?.(TaskStage.DATA_STORE_CONSOLIDATION, 'failed');
    }
  }
  ...
  private async fetchDbEntities(task: IndexRepositoryTask): Promise<Map<string, CanonicalEntity>> {
    ...
    for (const entityType of dbEntityTypes) {
      try {
        const dbEntities = await this.config.dataAdapter.listEntities(task.tenantId, entityType, 1000);
