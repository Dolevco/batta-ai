...
type AnyIngressGraph = IngressGraph | AWSIngressGraph;
type AnyNetworkTopology = NetworkTopology | AWSNetworkTopology;
type AnyIdentityGraph = IdentityGraph | AWSIdentityGraph;
...
export interface CloudGraphProvider {
   ...
   * their routing rules.
   */
  fetchIngressGraph(scope: ProviderScope): Promise<AnyIngressGraph>;
   ...
   */
  fetchNetworkTopology(scope: ProviderScope): Promise<AnyNetworkTopology>;

  /**
   * Fetch identity graph: managed identities, service principals, and role
   * assignments.
   */
  fetchIdentityGraph(scope: ProviderScope): Promise<AnyIdentityGraph>;
}
