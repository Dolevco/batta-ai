...
**TOPOLOGY CHECK (do this first):**
Before enumerating deps, determine the service's runtime boundary:
  - What can this process actually reach at runtime? Only list deps it calls DIRECTLY.
  - A dep that belongs to a different runtime (another process, a backend it calls over HTTP)
    is NOT a direct dep — record the intermediary (the API) as the single dep.
  - Env vars that point to a peer service (API URL, gateway address) indicate an api dep,
    not the infrastructure behind that service.
  - Auth/identity SDKs used client-side are identity deps — include them normally.
...
/**
 * NOTE ON TOPOLOGY BOUNDARIES — root cause prevention:
 *
 * Only deps reachable DIRECTLY by this service's process belong in its surface.
 * Deps behind a peer service (called over HTTP/API) are NOT direct deps — the peer
 * is the dep; its infrastructure is invisible to this service.
 *
 * The TOPOLOGY CHECK in customInstructions above is the primary guard.
 * buildSiblingsContext() reinforces this: only compiled-in library siblings propagate
 * their external deps; separately-deployed services are a single api dep.
 */
