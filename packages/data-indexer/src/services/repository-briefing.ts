 ...
 *
 * The briefing is:
 *   1. Stored on the CodeRepository entity (Qdrant + Neo4j) so it is visible
  ...
  private async persist(repo: CodeRepository): Promise<void> {
    ...
    if (this.qdrant) {
      try {
        ...
        console.error(
          `   [RepoBriefing] ⚠️  Qdrant: failed to persist briefing for "${repo.name}":`,
          err instanceof Error ? err.message : String(err),
        );
      }
    }
    ...
    if (this.neo4j) {
      try {
        ...
        console.error(
          `   [RepoBriefing] ⚠️  Neo4j: failed to persist briefing for "${repo.name}":`,
          err instanceof Error ? err.message : String(err),
        );
      }
    }
