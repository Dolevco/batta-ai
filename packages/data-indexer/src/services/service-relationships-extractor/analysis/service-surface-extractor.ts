...
export class ServiceSurfaceExtractor {
  ...
  async extractSurface(
    ...
    const skeletonContext = buildSkeletonContext(skeleton);
    const siblingsContext = buildSiblingsContext(skeleton.internalDependencies, knownSiblings);
    ...
    if (!result.requiredOutput) {
      ...
      return this.buildFallbackSurface(skeleton);
    }
  ...
  private buildFallbackSurface(skeleton: ServiceSkeleton): ServiceExternalSurface {
    return {
      externalDeps: [],
...
function buildSiblingsContext(
  ...
  const lines: string[] = [
    ...
    `This service depends on the following internal packages. Their external deps are`,
    `ALSO part of this service's external surface — include them in the output.`,
    '',
  ];
  ...
  for (const dep of internalDeps) {
    ...
    if (known && known.externalDeps.length > 0) {
      ...
      lines.push('  → Include ALL of the above as transitive deps in your output.');
    } else if (known) {
