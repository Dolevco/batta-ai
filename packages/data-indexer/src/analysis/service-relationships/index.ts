...
import type { BuildArtifact, CloudResource, CodeService, DeploymentArtifact, EntityId, Relationship, ServiceExternalSurface } from '@batta/shared';
...
import { emitExternalDepRelationships } from '../static-dependencies/relationship-emitter';
...
export class ServiceRelationshipsExtractor {
  ...
  constructor(
    ...
    this.persistence = new PersistenceHelper(graphAdapter, dataAdapter);
  ...
  /**
   * Fetch the cloud topology context for a single service from the graph and data stores.
   */
  ...
  async extract(input: ServiceRelationshipsInput): Promise<ServiceRelationshipsResult> {
    ...
    if (domainEnabled('services') && services.length > 0) {
      ...
      for (const service of serviceMap.values()) {
        try {
          const { relationships: depRels } = emitExternalDepRelationships(
      ...
      console.log(
        ...
        `(unmatched deps stored on service only, not emitted to graph)`,
      );
    }
    ...
    if (domainEnabled('services') && buildArtifacts.length > 0 && services.length > 0) {
      ...
      const rels = await correlateBuildToService(
        this.registry, this.persistence, tenantId, repositoryPath,
