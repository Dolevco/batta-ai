...
import { PlannedTaskCompletionTool, Task, MODES } from '@batta/core';
import type {
  CanonicalEntity,
  CodeRepository,
  CodeService,
...
export class RepositoryResponsibilityCalculator {
   ...
   * the entity in-place, and persist to data and graph stores.
   *
  ...
  private async persist(repo: CodeRepository): Promise<void> {
    ...
    if (this.dataAdapter) {
      try {
        ...
        console.error(
          `   [RepoResp] ⚠️  DataAdapter: failed to persist responsibility for "${repo.name}":`,
          err instanceof Error ? err.message : String(err),
        );
      }
    }
    ...
    if (this.graphAdapter) {
      try {
        ...
        console.error(
          `   [RepoResp] ⚠️  GraphAdapter: failed to persist responsibility for "${repo.name}":`,
          err instanceof Error ? err.message : String(err),
        );
      }
    }
