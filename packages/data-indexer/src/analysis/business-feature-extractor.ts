 ...
 *
 * Each step uses a Task from @batta/core (NOT a direct LLM call).
 * Custom completion tools validate the LLM output and retry on failure.
 *
 * Results are persisted to:
 *   - Data store : features + entities (serviceDfd / serviceThreatModel on CodeService)
 *   - Graph store: IMPLEMENTS_FEATURE edges (feature_analysis) → (code_service)
 *
 * Security: LLM outputs are stripped of workspace paths before storage.
 *           All writes include tenantId for tenant isolation.
 *           Validation errors contain only field-name / enum details — never
 *           raw user data, secrets, or workspace paths.
 */

import * as crypto from 'crypto';
import type { CloudResource, CodeService, Relationship, RepositoryBriefing, TenantId } from '@batta/shared';
import { PostgresDataAdapter, PostgresGraphAdapter } from '@batta/shared';
import type {
  BusinessFeature,
  DataFlowDiagram,
  DFDActor,
...
export class BusinessFeatureExtractor {
   ...
   * BusinessFeature[] (also persisted if adapters are provided).
   *
  ...
  private async persistFeatures(
    ...
    for (const feature of [...featuresToDeprecate, ...featuresToWrite]) {
      try {
        if (this.dataAdapter) {
          await this.dataAdapter.storeEntity({
            id: feature.id,
            tenantId,
        ...
        // Graph edge: only for active features (deprecated ones keep their existing edges)
        if (this.graphAdapter && feature.status === 'active') {
          await this.graphAdapter.storeRelationship({
    ...
    if (this.dataAdapter && activeFeaturesForSemanticIndex.length > 0) {
      try {
        const semanticDocs = activeFeaturesForSemanticIndex.map(f => buildFeatureSemanticDoc(tenantId, f));
        await this.dataAdapter.storeSemanticDocuments(semanticDocs);
        console.log(
   ...
   * Filters in-memory since the adapter doesn't support nested payload filters here.
   */
  private async loadActiveFeaturesByService(
    ...
    try {
      const all = await this.dataAdapter.listEntities(tenantId, 'feature_analysis' as any, 500);
      return all
  ...
  /**
   * Patches the CodeService entity in the data store with the service-level DFD,
   ...
   *  - Writes are scoped to tenantId (enforced by DataAdapter).
   *  - Only schema-level errors are logged; no raw LLM output is emitted.
   *  - The service-level risk score is written to graph store only as a numeric
  ...
  private async persistServiceSecurityData(
    ...
    try {
      ...
      const existing = await this.dataAdapter.getEntity(tenantId, service.id);
      ...
      await this.dataAdapter.storeEntity(updated as any);

      // Also update the service node in graph store so risk score and threat model are in sync (best-effort).
      if (this.graphAdapter) {
        try {
          await this.graphAdapter.storeEntity(updated as any);
        } catch (graphErr) {
          // Non-fatal: graph store enrichment is best-effort
          console.warn(
            `[BusinessFeatureExtractor] Graph store service node update failed for "${service.name}":`,
 ...
 */
function enforceSurfaceCoverage(
  dfd: DataFlowDiagram,
  serviceName: string,
  allDeps: ExternalDep[],
): DataFlowDiagram {
