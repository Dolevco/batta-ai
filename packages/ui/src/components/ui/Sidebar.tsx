...
const NAV_ITEMS: NavItem[] = [
  ...
  /*{ key: 'planner', icon: <ProjectOutlined />, label: 'Planner' },
  {
    ...
    children: [
      ...
      { key: 'agents:history',  icon: <HistoryOutlined />,     label: 'Run History' },
      { key: 'agents:settings', icon: <UserOutlined />,        label: 'Settings'    },
    ],
  },
  {
    key: 'knowledge-base', icon: <FolderOutlined />, label: 'Knowledge Base',
    children: [
...
export function Sidebar({ selected, onSelect, collapsed, onToggleCollapse }: {
  selected: string;
...
function NavBtn({
  icon, label, active, collapsed, activeBg, hoverBg, textMuted, indent, onClick,
}: {
  ...
  return (
    ...
    >
      <span style={{ fontSize: 18, color: active ? T.orange : textMuted, display: 'flex', flexShrink: 0 }}>{icon}</span>
      {!collapsed && (
        <span style={{ fontSize: 14, fontWeight: active ? 500 : 400, color: active ? T.orange : textMuted, whiteSpace: 'nowrap' }}>
          {label}
        </span>
      )}
    </button>
