 ...
 *
 * Design decisions:
 * - dfd.trustBoundaries seeds the available zones; empty zones are not rendered.
...
function buildLayout(
  ...
  for (const rawTb of (dfd.trustBoundaries ?? [])) {
    // Guard against object-shaped entries from older indexed data
    const tb: string = typeof rawTb === 'string' ? rawTb
      : rawTb && typeof rawTb === 'object' ? String((rawTb as Record<string, unknown>).type ?? (rawTb as Record<string, unknown>).name ?? '')
      : '';
  ...
  const visibleBoundaryNames = allBoundaryNames.filter((tb) => (boundaryContents.get(tb) ?? []).length > 0);

  const groupTotalHeights = visibleBoundaryNames.map((tb) => {
    const n = (boundaryContents.get(tb) ?? []).length;
    const inner = colHeight(n);
  ...
  visibleBoundaryNames.forEach((tbName, tbIdx) => {
