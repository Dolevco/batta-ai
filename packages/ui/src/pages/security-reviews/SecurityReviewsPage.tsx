...
import {
  ...
  GithubOutlined, BranchesOutlined, IssuesCloseOutlined,
} from '@ant-design/icons';
import { useNavigate } from 'react-router-dom';
import type { SecurityReview, SecurityReviewStatus, ReviewSource } from '../../types';
import AgentIcon from '../../components/icons/AgentIcon';
import { useSecurityReviews } from '../../hooks/useSecurityReviews';
import { useCapabilities } from '../../hooks/useCapabilities';
import { useTheme } from '../../hooks/useTheme';
import { T, D, dk } from '../../theme';
import { MCP_CONFIG_TEMPLATE } from '../../constants/agentSetup';
...
function SourceBadge({ source, issueKey, isDark }: { source?: ReviewSource; issueKey?: string; isDark: boolean }) {
  if (!source || source === 'code') return null;
  if (source === 'cloud') {
    return (
      <span style={{
        display: 'inline-flex', alignItems: 'center', gap: 3,
        fontSize: 10, fontWeight: 600, padding: '2px 7px', borderRadius: 6,
        color: T.blue, background: dk(isDark, T.blueLight, D.blueLight),
...
const STATUS_FILTER_OPTIONS: { key: SecurityReviewStatus | 'all'; label: string }[] = [
  { key: 'all', label: 'All' },
...
const SOURCE_FILTER_OPTIONS: { key: ReviewSource | 'all'; label: string }[] = [
  { key: 'all', label: 'All Sources' },
  { key: 'code', label: 'Code' },
  { key: 'cloud', label: 'Cloud' },
  { key: 'jira_work_item', label: 'Jira' },
];

// ── Work Item Review Modal ────────────────────────────────────────────────────
...
function WorkItemModal({
  ...
  previewWorkItem: (issueKey: string) => Promise<WorkItemPreview>;
  startWorkItemReview: (issueKey: string) => Promise<{ reviewId: string; status: string }>;
}) {
  ...
  const handleIssueKeyChange = (value: string) => {
    setIssueKey(value);
    setPreview(null);
    ...
    if (/^[A-Z][A-Z0-9]+-\d+$/.test(trimmed)) {
      debounceRef.current = setTimeout(async () => {
        ...
        try {
          const data = await previewWorkItem(trimmed);
          setPreview(data);
        } catch (e: any) {
          setPreviewError(e?.message ?? 'Could not fetch issue');
        } finally {
  ...
  const handleStart = async () => {
    ...
    setSubmitting(true);
    try {
      const result = await startWorkItemReview(trimmed);
      message.success('Work item review started — agent is analysing…');
      onStarted(result.reviewId);
    } catch (e: any) {
      message.error(e?.message ?? 'Failed to start review');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Modal
      open={open}
      title="Start Work Item Security Review"
      ...
      footer={[
        ...
        <Button
          key="start"
          type="primary"
          disabled={!issueKey.trim() || submitting}
          loading={submitting}
          onClick={handleStart}
        >
          Start Review →
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: '8px 0' }}>
        <div>
          <div style={{ fontSize: 12, fontWeight: 600, color: dk(isDark, T.stone600, D.textMuted), marginBottom: 6 }}>
            Jira Issue Key
          </div>
          <Input
            placeholder="e.g. ENG-42"
            value={issueKey}
            onChange={e => handleIssueKeyChange(e.target.value)}
            size="large"
            style={{ fontFamily: 'monospace', textTransform: 'uppercase' }}
          />
          <div style={{ fontSize: 11, color: T.stone400, marginTop: 4 }}>Fetches issue on input</div>
        </div>

        {previewLoading && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: T.stone400, fontSize: 12 }}>
            <Spin size="small" /> Fetching issue…
          </div>
        )}

        {previewError && (
          <div style={{ fontSize: 12, color: T.red, padding: '8px 12px', borderRadius: 8, background: dk(isDark, T.redLight, D.redLight) }}>
            {previewError}
          </div>
        )}
        ...
        {preview && (
          <div style={{
            ...
            <div style={{ fontSize: 11, fontWeight: 700, color: T.blue, letterSpacing: '0.04em', textTransform: 'uppercase' }}>
              {preview.issueType} · {preview.projectKey}
            </div>
            <div style={{ fontSize: 13, fontWeight: 600, color: dk(isDark, T.stone800, D.text), lineHeight: 1.4 }}>
              {preview.summary}
            </div>
            <div style={{ display: 'flex', gap: 12, fontSize: 11, color: dk(isDark, T.stone500, D.textMuted), marginTop: 2 }}>
              {preview.assignee && <span>Assignee: {preview.assignee}</span>}
              {preview.priority && <span>Priority: {preview.priority}</span>}
            </div>
            {preview.components && preview.components.length > 0 && (
              <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginTop: 2 }}>
                {preview.components.map(c => (
                  <span key={c} style={{ fontSize: 10, fontWeight: 600, color: T.blue, background: 'rgba(255,255,255,0.6)', padding: '1px 6px', borderRadius: 4 }}>{c}</span>
                ))}
              </div>
            )}
          </div>
...
export function SecurityReviewsPage() {
  ...
  const { listReviews, loading, error, previewWorkItem, startWorkItemReview } = useSecurityReviews();
  const { byId: capabilityById } = useCapabilities();
  ...
  const getReviewSource = (r: SecurityReview): ReviewSource => r.source ?? 'code';
  const autonomousWorkItemReview = capabilityById.get('autonomousWorkItemReview');
  const reviewWorkItemDisabled = autonomousWorkItemReview?.available === false;
  const reviewWorkItemDisabledReason = reviewWorkItemDisabled
    ? autonomousWorkItemReview.reasons.join('. ')
    : undefined;

  const filtered = reviews.filter(r => {
    if (statusFilter !== 'all' && r.status !== statusFilter) return false;
    if (sourceFilter !== 'all' && getReviewSource(r) !== sourceFilter) return false;
    return true;
  });
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, width: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <Button
            ...
            type="primary"
            disabled={reviewWorkItemDisabled}
            title={reviewWorkItemDisabledReason}
      ...
      <div style={{
        ...
        <div style={{
          ...
          {/* Divider */}
          <span style={{ width: 1, height: 20, background: dk(isDark, T.stone200, D.border), margin: '0 4px' }} />
          ...
          {SOURCE_FILTER_OPTIONS.map(opt => {
            ...
            const count = opt.key === 'all'
              ? reviews.length
              : reviews.filter(r => getReviewSource(r) === opt.key).length;
            return (
              <button
                key={opt.key}
                ...
                style={{
                  ...
                  fontWeight: active ? 600 : 500, cursor: 'pointer',
                  border: active ? 'none' : `1px solid ${dk(isDark, T.stone200, D.borderSub)}`,
                  background: active ? T.blue : dk(isDark, T.white, D.bgSub),
                  color: active ? T.white : dk(isDark, T.stone500, D.textMuted),
                  display: 'flex', alignItems: 'center', gap: 5,
                  transition: 'all 0.15s',
                }}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 1000 }}>
            <thead>
              <tr style={{ borderBottom: `1px solid ${dk(isDark, T.stone100, D.border)}` }}>
                {['Feature', 'Source', 'Initiated By', 'Human Responsible', 'Repository', 'Services & Features', 'Status', 'Tasks', 'Compliance', 'Created', 'Completed'].map((col, i) => (
                  <th key={i} style={{
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={11} style={{ padding: '48px 0', textAlign: 'center' }}>
                    <EmptyReviewState isDark={isDark} onOpenOnboarding={() => navigate('/onboarding')} />
                  </td>
                </tr>
              ) : filtered.map(review => (
      ...
      <WorkItemModal
        ...
        isDark={isDark}
        onClose={() => setWorkItemModalOpen(false)}
        onStarted={(reviewId) => {
          setWorkItemModalOpen(false);
          navigate(`/knowledge-base/security-reviews/${reviewId}`);
        }}
        previewWorkItem={previewWorkItem}
        startWorkItemReview={startWorkItemReview}
      />
    </div>
  );
}

function EmptyReviewState({ isDark, onOpenOnboarding }: { isDark: boolean; onOpenOnboarding: () => void }) {
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
      <div style={{ width: 46, height: 46, borderRadius: 8, background: dk(isDark, T.orangeLight, D.orangeLight), display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <SafetyOutlined style={{ color: T.orange, fontSize: 22 }} />
      </div>
      <div>
        <div style={{ fontSize: 16, fontWeight: 700, color: dk(isDark, T.stone800, D.text) }}>No security reviews yet</div>
        <div style={{ fontSize: 12, color: dk(isDark, T.stone500, D.textMuted), marginTop: 4 }}>
          Recommended first run: connect a coding agent, run <Text code>/batta-index</Text>, add the batta rules to your agent instructions, then run <Text code>/batta-review &lt;feature&gt;</Text> for each change.
        </div>
      </div>
      <div style={{
        width: '100%',
        ...
        <Text copyable={{ text: MCP_CONFIG_TEMPLATE }} style={{ color: '#E5E7EB', whiteSpace: 'pre', fontFamily: 'monospace', fontSize: 12 }}>
          {MCP_CONFIG_TEMPLATE}
        </Text>
      </div>
      <Button type="primary" icon={<ApiOutlined />} onClick={onOpenOnboarding}>
        Open Agent Setup
      </Button>
    </div>
  );
...
function ReviewRow({ review, onNavigate, isDark }: { review: SecurityReview; onNavigate: (path: string) => void; isDark: boolean }) {
  ...
  return (
    ...
    >
      {/* Feature */}
      <td style={{ padding: '13px 16px', minWidth: 320, maxWidth: 420 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <span style={{
            fontSize: 13, fontWeight: 600, color: dk(isDark, T.stone800, D.text),
      ...
      {/* Source */}
      <td style={{ padding: '13px 16px', width: 100 }}>
        <SourceBadge source={review.source} issueKey={review.jiraWorkItemContext?.issueKey} isDark={isDark} />
      </td>
      ...
      <td style={{ padding: '13px 16px', width: 160 }}>
        ...
        ) : review.source === 'jira_work_item' ? (
          <span style={{ fontSize: 12, color: dk(isDark, T.stone500, D.textMuted) }}>Agent</span>
        ) : (
