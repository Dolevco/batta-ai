...
import {
  ...
  Modal, Form, Select, Checkbox, message,
} from 'antd';
import {
  ...
  ExportOutlined, ReloadOutlined, IssuesCloseOutlined,
...
import type {
  ...
  ExportToJiraOptions, SecurityRequirementExplanation,
} from '../../types';
...
import { ArchitectureDiffPanel } from '../../components/security-reviews/ArchitectureDiffPanel';
...
// ── Jira Work Item context panel ──────────────────────────────────────────────

function WorkItemContextPanel({ review, isDark }: { review: SecurityReview; isDark: boolean }) {
  const ctx = review.jiraWorkItemContext;
  if (!ctx) return null;

  return (
    <div style={{
      background: dk(isDark, '#EFF6FF', '#1e2d45'),
      border: `1px solid ${dk(isDark, '#BFDBFE', '#2d4a6e')}`,
      borderRadius: 12, padding: '16px 20px', marginBottom: 24,
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
        <IssuesCloseOutlined style={{ color: '#2563EB', fontSize: 15 }} />
        <span style={{ fontSize: 13, fontWeight: 700, color: '#2563EB' }}>Jira Work Item</span>
        <a
          href={ctx.issueUrl}
          target="_blank"
          rel="noopener noreferrer"
          style={{
            fontSize: 11, padding: '1px 8px', borderRadius: 4, marginLeft: 4,
            background: dk(isDark, '#DBEAFE', 'rgba(37,99,235,0.2)'),
            border: `1px solid ${dk(isDark, '#BFDBFE', 'rgba(37,99,235,0.4)')}`,
        ...
        <span style={{
          ...
          color: T.stone500, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em',
        }}>
          {ctx.issueType}
        </span>
      </div>

      <div style={{ fontSize: 14, fontWeight: 600, color: dk(isDark, T.stone900, D.text), marginBottom: 10 }}>
        {ctx.summary}
      </div>

      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, fontSize: 12, color: dk(isDark, T.stone600, D.textMuted) }}>
        {ctx.projectKey && (
          <span><span style={{ fontWeight: 600, color: dk(isDark, T.stone500, T.stone400) }}>Project:</span> {ctx.projectKey}</span>
        )}
        {ctx.assignee && (
          <span><span style={{ fontWeight: 600, color: dk(isDark, T.stone500, T.stone400) }}>Assignee:</span> {ctx.assignee}</span>
        )}
        {ctx.reporter && (
          <span><span style={{ fontWeight: 600, color: dk(isDark, T.stone500, T.stone400) }}>Reporter:</span> {ctx.reporter}</span>
        )}
        {ctx.priority && (
          <span><span style={{ fontWeight: 600, color: dk(isDark, T.stone500, T.stone400) }}>Priority:</span> {ctx.priority}</span>
        )}
      </div>

      {(ctx.labels?.length || ctx.components?.length) && (
      ...
      {ctx.linkedPrUrl && (
        <div style={{ marginTop: 10, fontSize: 12 }}>
          <span style={{ fontWeight: 600, color: dk(isDark, T.stone500, T.stone400) }}>Linked PR: </span>
          <a href={ctx.linkedPrUrl} target="_blank" rel="noopener noreferrer" style={{ color: T.blue }}>
            {ctx.linkedPrUrl}
          </a>
        </div>
      )}
...
function AgentStatusBanner({ review, onRetry, isDark }: { review: SecurityReview; onRetry: () => void; isDark: boolean }) {
  if (review.source !== 'jira_work_item') return null;

  if (review.agentError) {
    return (
      <div style={{
        ...
        <ExclamationCircleOutlined style={{ color: T.red, fontSize: 16, flexShrink: 0 }} />
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: T.red }}>Agent analysis failed</div>
          <div style={{ fontSize: 12, color: dk(isDark, T.stone600, D.textMuted), marginTop: 2, fontFamily: 'monospace' }}>
            {review.agentError}
          </div>
        </div>
        <Button
          ...
          icon={<ReloadOutlined />}
          onClick={onRetry}
          style={{ flexShrink: 0, borderColor: T.red, color: T.red }}
        >
          Retry Agent
        </Button>
      </div>
  ...
  if (review.agentStatus === 'pending' || review.agentStatus === 'running') {
    return (
      <div style={{
        background: dk(isDark, T.blueLight, D.blueLight),
        border: `1px solid ${dk(isDark, T.blueBorder, D.blueBorder)}`,
        ...
        <SyncOutlined spin style={{ color: T.blue, fontSize: 15, flexShrink: 0 }} />
        <div>
          <div style={{ fontSize: 13, fontWeight: 600, color: T.blue }}>
            {review.agentStatus === 'running' ? 'Agent is analysing this work item…' : 'Analysis queued — agent will start shortly'}
          </div>
          <div style={{ fontSize: 12, color: dk(isDark, T.stone500, D.textMuted), marginTop: 2 }}>
            Security questionnaire answers will be populated automatically.
          </div>
        </div>
      </div>
    );
...
function ExportToJiraModal({ open, onClose, onExport }: {
  open: boolean;
  onClose: () => void;
  onExport: (opts: ExportToJiraOptions) => Promise<void>;
}) {
  ...
  const handleOk = async () => {
    try {
      ...
      setExporting(true);
      await onExport({
        targetProjectKey: values.targetProjectKey.trim().toUpperCase(),
        severityThreshold: values.severityThreshold,
        issueType: values.issueType || 'Task',
        onlyUnhandled: values.onlyUnhandled,
      });
      form.resetFields();
      onClose();
    } catch (err: any) {
  ...
  return (
    <Modal
      title={<span style={{ display: 'flex', alignItems: 'center', gap: 8 }}><IssuesCloseOutlined style={{ color: '#2563EB' }} /> Export Tasks to Jira</span>}
      open={open}
      onCancel={onClose}
      onOk={handleOk}
      okText="Export"
      ...
      destroyOnClose
    >
      <Form form={form} layout="vertical" initialValues={{ severityThreshold: 'high', issueType: 'Task', onlyUnhandled: true }}>
        <Form.Item
          label="Target Jira Project Key"
          name="targetProjectKey"
          rules={[
            { required: true, message: 'Project key is required' },
            { pattern: /^[A-Z][A-Z0-9]+$/, message: 'Must be a valid project key (e.g. ENG)' },
          ]}
        >
          <Input placeholder="ENG" style={{ textTransform: 'uppercase' }} />
        </Form.Item>
        <Form.Item label="Minimum Severity" name="severityThreshold">
          <Select>
            <Select.Option value="critical">Critical only</Select.Option>
            <Select.Option value="high">High and above</Select.Option>
            <Select.Option value="medium">Medium and above</Select.Option>
            <Select.Option value="low">All severities</Select.Option>
          </Select>
        </Form.Item>
        <Form.Item label="Issue Type" name="issueType">
          <Input placeholder="Task" />
        </Form.Item>
        <Form.Item name="onlyUnhandled" valuePropName="checked">
          <Checkbox>Skip tasks already exported to Jira</Checkbox>
        </Form.Item>
      </Form>
    </Modal>
  );
...
function OverviewSection({ review, summary, isDark, onRetryAgent }: { review: SecurityReview; summary: SecurityReviewAttestationSummary | null; isDark: boolean; onRetryAgent: () => void }) {
  const navigate = useNavigate();
  const isWorkItem = review.source === 'jira_work_item';
  ...
  const exportedCount  = review.tasks.filter(t => t.jiraIssueKey).length;
  ...
  return (
    <div>
      <SectionHeader
        ...
        subtitle={isWorkItem ? 'Jira work item security analysis' : 'Full lifecycle status of this security review'}
      />

      {/* Jira work item context panel — prominent for work items */}
      <WorkItemContextPanel review={review} isDark={isDark} />

      {/* Agent status banner */}
      <AgentStatusBanner review={review} onRetry={onRetryAgent} isDark={isDark} />

      {/* Work item: compact status card instead of stepper */}
      {isWorkItem ? (
        <div style={{
          ...
          <div style={{ flex: 1, minWidth: 160 }}>
            <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase' as const, letterSpacing: '0.06em', color: T.stone400, marginBottom: 4 }}>Status</div>
            <span style={{
              display: 'inline-flex', alignItems: 'center', gap: 5,
              ...
              {workItemStatusLabel(review)}
            </span>
          </div>
          {[
            { label: 'Action Items', value: review.tasks.length, color: dk(isDark, T.stone800, D.text) },
            { label: 'Critical',     value: criticalCount,       color: criticalCount > 0 ? T.red : T.green },
            { label: 'Exported',     value: exportedCount,       color: exportedCount > 0 ? '#2563EB' : T.stone400 },
          ].map(stat => (
            <div key={stat.label} style={{
              ...
              <span style={{ fontSize: 10, fontWeight: 600, color: T.stone400, textTransform: 'uppercase' as const, letterSpacing: '0.05em' }}>{stat.label}</span>
            </div>
          ))}
          {review.agentCompletedAt && (
            <div style={{ fontSize: 11, color: T.stone400 }}>
              Analysis completed<br />{formatDate(review.agentCompletedAt)}
            </div>
          )}
        </div>
      ) : (
        /* Code review: 4-step stepper */
        <div style={{
          background: dk(isDark, T.stone50, D.bgSub),
          border: `1px solid ${dk(isDark, T.stone200, D.borderSub)}`,
      ...
      {/* Key stats row — only shown for code reviews (work items have the compact card above) */}
      {!isWorkItem && <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}>
        {[
      ...
      {/* Compliance bar — code reviews only */}
      {!isWorkItem && compliancePct !== null && (
        <div style={{
          background: dk(isDark, T.white, D.bgCard),
          border: `1px solid ${dk(isDark, T.stone200, D.borderSub)}`,
...
function QuestionnaireSection({ review, isDark }: { review: SecurityReview; isDark: boolean }) {
  ...
  // Work item quality metrics
  const highConf    = review.answers.filter(a => a.confidence === 'high').length;
  const medConf     = review.answers.filter(a => a.confidence === 'medium').length;
  const lowConf     = review.answers.filter(a => a.confidence === 'low').length;
  const unknownCount = review.answers.filter(a => {
    const ans = a.answer.trim().toLowerCase();
    return ans.startsWith('unknown') || ans.startsWith('n/a') || ans === '';
  }).length;
  const needsAttention = lowConf + unknownCount;

  return (
    <div>
      <SectionHeader
        isDark={isDark}
        title={isWorkItem ? 'Agent Analysis' : 'Security Questionnaire'}
        subtitle={isWorkItem
          ? 'Autonomous security analysis of the Jira work item'
          : 'Agent-answered pre-flight questions that shaped the security task list'}
      />

      {/* Work item: analysis quality summary */}
      {isWorkItem && answered > 0 && (
        <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap' as const }}>
          {[
            { label: 'High confidence', value: highConf,  color: T.green, bg: dk(isDark, T.greenLight, D.greenLight), border: dk(isDark, T.greenBorder, D.greenBorder) },
            { label: 'Medium confidence', value: medConf, color: T.amber, bg: dk(isDark, T.amberLight, D.amberLight), border: dk(isDark, T.amberBorder, D.amberBorder) },
            { label: 'Low confidence',  value: lowConf,   color: T.red,   bg: dk(isDark, T.redLight, D.redLight),     border: dk(isDark, T.redBorder, D.redBorder) },
            { label: 'Unknown / N/A',   value: unknownCount, color: T.stone400, bg: dk(isDark, T.stone100, D.bgCard), border: dk(isDark, T.stone200, D.borderSub) },
          ].map(s => (
          ...
          {needsAttention > 0 && (
            <div style={{
              ...
              background: dk(isDark, T.amberLight, D.amberLight),
              border: `1px solid ${dk(isDark, T.amberBorder, D.amberBorder)}`,
            }}>
              <WarningOutlined style={{ color: T.amber }} />
              <span style={{ fontSize: 12, color: T.amber, fontWeight: 600 }}>
                {needsAttention} answer{needsAttention !== 1 ? 's' : ''} need reviewer attention
              </span>
            </div>
          )}
        </div>
      ...
      {answered === 0 && (
        <div style={{
          ...
          <div>
            <div style={{ fontSize: 13, fontWeight: 600, color: T.blue }}>
              {isWorkItem ? 'Agent analysis not yet complete' : 'Questionnaire not yet answered'}
            </div>
            <div style={{ fontSize: 12, color: dk(isDark, T.stone500, D.textMuted), marginTop: 2 }}>
              {isWorkItem
                ? 'Security questions will be answered automatically when agent analysis runs.'
                ...
                <div style={{ flex: 1 }}>
                  ...
                  {ans ? (
                    <div style={{ marginTop: 8 }}>
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                        ...
                        {ans.confidence && (
                          <span style={{
                            ...
                            background: dk(isDark, T.stone100, D.bgSub),
                            border: `1px solid ${dk(isDark, T.stone200, D.borderSub)}`,
                            color: ans.confidence === 'high' ? T.green : ans.confidence === 'medium' ? T.amber : T.stone400,
                      ...
                      {ans.rationale && (
                        <div style={{ marginTop: 6, fontSize: 12, color: dk(isDark, T.stone600, D.textMuted), lineHeight: 1.6 }}>
                          {ans.rationale}
                        </div>
                      )}
...
function TasksSection({ review, isDark, onReviewUpdate: _onReviewUpdate }: { review: SecurityReview; isDark: boolean; onReviewUpdate: (r: SecurityReview) => void }) {
  const { explainSecurityRequirement } = useSecurityReviews();
  const [explanation, setExplanation] = useState<SecurityRequirementExplanation | null>(null);
  const [explanationOpen, setExplanationOpen] = useState(false);
  const [explanationLoading, setExplanationLoading] = useState(false);
  const isWorkItem = review.source === 'jira_work_item';
  const title    = isWorkItem ? 'Security Action Items' : 'Security Tasks';
  const subtitle = isWorkItem ? 'Action items generated by agent analysis' : 'Tasks generated by the security review engine';

  if (review.tasks.length === 0) {
    return (
      <div>
        <SectionHeader isDark={isDark} title={title} subtitle={subtitle} />
        <div style={{
          background: dk(isDark, T.blueLight, D.blueLight),
          border: `1px solid ${dk(isDark, T.blueBorder, D.blueBorder)}`,
          borderRadius: 10,
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: dk(isDark, T.stone500, D.textMuted), marginTop: 2 }}>
              {isWorkItem ? 'Action items are generated after agent analysis completes.' : 'Tasks are generated after the questionnaire is answered.'}
            </div>
          </div>
        </div>
      </div>
  ...
  const exportedCount   = review.tasks.filter(t => t.jiraIssueKey).length;
  const unexportedCount = review.tasks.length - exportedCount;

  const openExplanation = async (taskId: string) => {
    setExplanationOpen(true);
    setExplanation(null);
    setExplanationLoading(true);
    try {
      setExplanation(await explainSecurityRequirement(review.id, taskId));
    } catch (err: any) {
      message.error(err?.message ?? 'Failed to explain requirement');
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: dk(isDark, T.stone900, D.text) }}>{title}</h3>
          <p style={{ margin: '3px 0 0', fontSize: 12, color: T.stone400 }}>
            {`${review.tasks.length} task${review.tasks.length !== 1 ? 's' : ''} identified — ordered by severity`}
            {isWorkItem && exportedCount > 0 && ` · ${exportedCount} exported to Jira`}
            {isWorkItem && unexportedCount > 0 && ` · ${unexportedCount} pending export`}
          </p>
        </div>
      </div>
              ...
              <div style={{ padding: '14px 18px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' as const }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, flexWrap: 'wrap' as const }}>
                      ...
                      {task.jiraIssueKey && (
                        ...
                        >
                          <IssuesCloseOutlined style={{ fontSize: 9 }} />
                          {task.jiraIssueKey} ↗
                        </a>
                      )}
                    </div>
                  ...
                  <div style={{ flexShrink: 0, display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                    <Button size="small" icon={<QuestionCircleOutlined />} onClick={() => openExplanation(task.id)}>
                      Why required?
                    </Button>
                    {isHandled ? (
                      <span style={{
      ...
      <Modal
        ...
        title="Why this task is required"
        onCancel={() => setExplanationOpen(false)}
        footer={<Button onClick={() => setExplanationOpen(false)}>Close</Button>}
        width={680}
      >
        {explanationLoading || !explanation ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div>
              <SeverityBadge severity={explanation.task.severity} isDark={isDark} />
              <h3 style={{ margin: '8px 0 4px', fontSize: 15, color: dk(isDark, T.stone900, D.text) }}>{explanation.task.title}</h3>
              <p style={{ margin: 0, fontSize: 12, color: dk(isDark, T.stone500, D.textMuted), lineHeight: 1.6 }}>
                {explanation.reason}
              </p>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 10 }}>
              <MetaMini label="Policy" value={`${explanation.policy.type ?? 'security_review'}${explanation.policy.version ? ` v${explanation.policy.version}` : ''}`} isDark={isDark} />
              <MetaMini label="Source" value={explanation.policy.source === 'question_rule' ? `Question ${explanation.policy.questionId}` : 'Baseline task'} isDark={isDark} />
            </div>
            {explanation.triggeringAnswers.length > 0 && (
              <div>
                <div style={{ fontSize: 11, fontWeight: 700, color: T.stone400, textTransform: 'uppercase', marginBottom: 6 }}>Triggering answer</div>
                {explanation.triggeringAnswers.map(answer => (
                  <div key={answer.questionId} style={{ padding: 10, borderRadius: 8, background: dk(isDark, T.stone50, D.bgSub), fontSize: 12, color: dk(isDark, T.stone700, D.textMuted), lineHeight: 1.6 }}>
                    <Text code>{answer.questionId}</Text> {answer.answer}
                  </div>
                ))}
              </div>
            )}
            {explanation.relatedContext.featureSecurityContext.length > 0 && (
              <div>
                <div style={{ fontSize: 11, fontWeight: 700, color: T.stone400, textTransform: 'uppercase', marginBottom: 6 }}>Related indexed context</div>
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {explanation.relatedContext.featureSecurityContext.map(ctx => (
                    <span key={ctx.featureId} style={{
...
function MetaMini({ label, value, isDark }: { label: string; value: string; isDark: boolean }) {
  return (
    <div style={{ padding: 10, borderRadius: 8, background: dk(isDark, T.stone50, D.bgSub), border: `1px solid ${dk(isDark, T.stone200, D.borderSub)}` }}>
      <div style={{ fontSize: 10, fontWeight: 700, color: T.stone400, textTransform: 'uppercase', marginBottom: 3 }}>{label}</div>
      <div style={{ fontSize: 12, color: dk(isDark, T.stone700, D.text) }}>{value}</div>
    </div>
  );
}
...
const POLICY_META: Record<PolicyTemplateType, { label: string; icon: React.ReactNode; standard: string; description: string }> = {
  ...
  work_item_review: {
    ...
    standard: 'Internal',
    description: 'Security controls applied automatically when a Jira work item is reviewed by the agent.',
  },
};
...
function PoliciesSection({ review, policies, isDark }: { review: SecurityReview; policies: PolicyTemplate[]; isDark: boolean }) {
  ...
  const allTypes: PolicyTemplateType[] = review.source === 'jira_work_item'
    ? ['work_item_review']
    : ['security_review', 'responsible_ai', 'privacy'];
...
// ── Work item status helpers ──────────────────────────────────────────────────

function workItemStatusLabel(review: SecurityReview): string {
  if (review.agentStatus === 'pending') return 'Queued';
  if (review.agentStatus === 'running') return 'Analysing…';
  if (review.agentStatus === 'failed')  return 'Analysis Failed';
  switch (review.status) {
    case 'questionnaire_pending':  return 'Queued';
    case 'questionnaire_answered': return 'Action items generated';
    case 'tasks_acknowledged':     return 'Ready to export';
    case 'attested':               return 'Completed';
...
function JiraExportSection({ review, isDark, onReviewUpdate }: {
  review: SecurityReview; isDark: boolean; onReviewUpdate: (r: SecurityReview) => void;
}) {
  const { exportToJira } = useSecurityReviews();
  const [exportModalOpen, setExportModalOpen] = useState(false);
  const [exportResult, setExportResult] = useState<{ exported: number; failed: number } | null>(null);

  const handleExport = async (opts: ExportToJiraOptions) => {
    const result = await exportToJira(review.id, opts);
    setExportResult({ exported: result.exported, failed: result.failed });
    onReviewUpdate({ ...review, tasks: result.tasks });
  };

  const exportedTasks   = review.tasks.filter(t => t.jiraIssueKey);
  const unexportedTasks = review.tasks.filter(t => !t.jiraIssueKey);
  const severityOrder   = { critical: 0, high: 1, medium: 2, low: 3 };

  const unexportedBySeverity = (['critical', 'high', 'medium', 'low'] as const).map(sev => ({
    sev,
    tasks: unexportedTasks.filter(t => t.severity === sev),
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
        <SectionHeader
          isDark={isDark}
          title="Jira Export"
          subtitle="Export security action items as Jira issues"
        />
        <Button
          type="primary"
          icon={<ExportOutlined />}
          onClick={() => setExportModalOpen(true)}
          disabled={review.tasks.length === 0}
      ...
      {review.jiraExportError && (
        <div style={{
          ...
          <ExclamationCircleOutlined style={{ color: T.red, marginTop: 2, flexShrink: 0 }} />
          <div>
            <div style={{ fontSize: 13, fontWeight: 600, color: T.red }}>Jira export error</div>
            <div style={{ fontSize: 12, color: dk(isDark, T.stone600, D.textMuted), marginTop: 2, fontFamily: 'monospace' }}>
              {review.jiraExportError}
            </div>
          </div>
      ...
      {exportResult && (
        <div style={{
          ...
          <CheckCircleOutlined style={{ color: T.green }} />
          <span style={{ color: T.green, fontWeight: 600 }}>
            {exportResult.exported} task{exportResult.exported !== 1 ? 's' : ''} exported to Jira
            {exportResult.failed > 0 && ` (${exportResult.failed} failed)`}
          </span>
          <button onClick={() => setExportResult(null)} style={{ marginLeft: 'auto', background: 'none', border: 'none', cursor: 'pointer', color: T.stone400, fontSize: 14, lineHeight: 1 }}>×</button>
        </div>
      )}
      ...
      {/* Export history */}
      {exportedTasks.length > 0 && (
        <div style={{ marginBottom: 24 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: T.stone400, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 10 }}>
            Exported ({exportedTasks.length})
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[...exportedTasks].sort((a, b) => (severityOrder[a.severity] ?? 4) - (severityOrder[b.severity] ?? 4)).map(task => {
              const cfg = SEVERITY_CONFIG[task.severity];
              return (
                <div key={task.id} style={{
                  ...
                  <SeverityBadge severity={task.severity} isDark={isDark} />
                  <span style={{ flex: 1, fontSize: 13, color: dk(isDark, T.stone800, D.text), fontWeight: 500 }}>{task.title}</span>
                  <a
                    href={task.jiraIssueUrl}
                    target="_blank"
                    rel="noopener noreferrer"
      ...
      {/* Unexported by severity */}
      {unexportedBySeverity.length > 0 && (
        <div>
          <div style={{ fontSize: 12, fontWeight: 700, color: T.stone400, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 10 }}>
            Pending export ({unexportedTasks.length})
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {unexportedBySeverity.map(({ sev, tasks }) => {
              const cfg = SEVERITY_CONFIG[sev];
              return (
                <div key={sev}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                    <SeverityBadge severity={sev} isDark={isDark} />
                    <span style={{ fontSize: 11, color: dk(isDark, T.stone400, D.textFaint) }}>{tasks.length} task{tasks.length !== 1 ? 's' : ''}</span>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {tasks.map(task => (
                      <div key={task.id} style={{
                        ...
                        border: `1px solid ${dk(isDark, T.stone200, D.borderSub)}`,
                        borderLeft: `3px solid ${cfg.color}`,
                        fontSize: 13, color: dk(isDark, T.stone800, D.text),
                      }}>
                        {task.title}
                      </div>
                    ))}
      ...
      {review.tasks.length === 0 && (
        <div style={{
          ...
          No security action items yet — run agent analysis first.
        </div>
      )}
    </div>
  );
...
type SectionId = 'overview' | 'questionnaire' | 'tasks' | 'attestations' | 'summary' | 'threat-model' | 'policies' | 'pr-correlation' | 'pr-validation' | 'jira-export';
...
const NAV_ITEMS: NavItem[] = [
  ...
  },
  {
    ...
    group: 'Integrations',
    getBadge: (r) => {
      const exported = r.tasks.filter(t => t.jiraIssueKey).length;
      return exported > 0 ? { count: exported, color: '#2563EB' } : null;
    },
  },
];
...
export function SecurityReviewDetailsPage() {
  ...
  const { getReview, getAttestationSummary, retryWorkItemReview, loading, error } = useSecurityReviews();
  ...
  // Filter nav based on source
  const visibleNavItems = NAV_ITEMS.filter(item => {
    if (!isWorkItem) {
      // Code reviews: hide jira-export
      return item.id !== 'jira-export';
    }
    // Work item reviews: hide code-review-specific sections
    return !['threat-model', 'pr-correlation', 'pr-validation', 'attestations'].includes(item.id);
  });

  // Section label overrides for work item reviews
  const sectionLabel = (id: SectionId): string => {
    if (isWorkItem) {
      if (id === 'questionnaire') return 'Agent Analysis';
      if (id === 'tasks') return 'Security Action Items';
    }
    return visibleNavItems.find(n => n.id === id)?.label ?? id;
  };
      ...
      <div style={{ padding: '0 0 16px', flexShrink: 0, display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <Breadcrumb
          items={[
            ...
            { title: review.title ?? review.jiraWorkItemContext?.summary },
          ]}
        />
      </div>
      ...
      <div style={{ display: 'flex', flex: 1, minHeight: 0, overflow: 'hidden' }}>
        ...
        <aside style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
              <div style={{
                ...
                background: isWorkItem ? '#2563EB' : T.orange,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                color: '#fff', fontSize: 14,
              }}>
                {isWorkItem ? <IssuesCloseOutlined /> : <SafetyOutlined />}
              </div>
              {isWorkItem ? (
                <span style={{
                  display: 'inline-flex', alignItems: 'center', gap: 5,
            ...
            <div style={{
              ...
              {review.title ?? review.jiraWorkItemContext?.summary}
            </div>
            {review.agentName && (
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '6px 0' }}>
            {visibleNavItems.map((item, i) => {
              ...
              const prevGroup = i > 0 ? visibleNavItems[i - 1].group : null;
              const showGroup = item.group !== prevGroup;
              const badge = item.getBadge?.(review);
              const label = isWorkItem && item.id === 'questionnaire' ? 'Agent Analysis'
                          : isWorkItem && item.id === 'tasks' ? 'Action Items'
                          : item.label;
          ...
          <div style={{
            ...
            {isWorkItem && review.agentCompletedAt && (
              <div style={{ marginTop: 2 }}>Analysis {formatDate(review.agentCompletedAt)}</div>
            )}
          </div>
        </aside>
        ...
        <div style={{
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: '24px 28px' }}>
            {activeSection === 'overview'        && <OverviewSection      review={review} summary={summary} isDark={isDark} onRetryAgent={async () => { await retryWorkItemReview(review.id); const r = await getReview(review.id); setReview(r); }} />}
            {activeSection === 'questionnaire'   && <QuestionnaireSection review={review} isDark={isDark} />}
            {activeSection === 'tasks'           && <TasksSection         review={review} isDark={isDark} onReviewUpdate={setReview} />}
            ...
            {activeSection === 'jira-export'     && <JiraExportSection    review={review} isDark={isDark} onReviewUpdate={setReview} />}
          </div>
        </div>
      </div>
    </div>
