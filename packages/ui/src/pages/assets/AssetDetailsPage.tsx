...
import { ExploitabilityPanel } from '../../components/assets/ExploitabilityPanel';
          ...
          <div>
            ...
            {!!asset.metadata?.principalId && (
              <InfoRow label="Object ID (Principal)" T={T}>
                <span style={{ fontFamily: 'monospace', fontSize: 10, color: T.stone600, wordBreak: 'break-all' }}>
                  {asset.metadata.principalId as string}
                </span>
              </InfoRow>
            )}
            {!!asset.metadata?.clientId && (
              <InfoRow label="Client ID (App ID)" T={T}>
                <span style={{ fontFamily: 'monospace', fontSize: 10, color: T.stone600, wordBreak: 'break-all' }}>
                  {asset.metadata.clientId as string}
                </span>
              </InfoRow>
            )}
            {!!asset.metadata?.region && (
              <InfoRow label="Region" T={T}>{asset.metadata.region as string}</InfoRow>
            )}
            {!!asset.metadata?.resourceId && (
              <InfoRow label="ARM Resource ID" T={T}>
                <span style={{ fontFamily: 'monospace', fontSize: 10, color: T.stone500, wordBreak: 'break-all' }}>
                  {asset.metadata.resourceId as string}
                </span>
              </InfoRow>
            )}
            {!!asset.metadata?.resourceGroup && (
              <InfoRow label="Resource Group" T={T}>{asset.metadata.resourceGroup as string}</InfoRow>
            )}
          </div>
        </Section>
      )}
...
function ServiceSecurityTab({ asset, T, RISK_COLORS, isDark }: { asset: AssetDetail; T: Tokens; RISK_COLORS: RiskColors; isDark: boolean }) {
  ...
  return (
    <div style={{ padding: '12px 0', display: 'flex', flexDirection: 'column', gap: 0 }}>
      ...
      {/* Generated at */}
      {!!(asset.serviceThreatModel as any)?.generatedAt && (
        <div style={{ padding: '8px 16px', fontSize: 10, color: T.stone300, borderTop: `1px solid ${secBorder}` }}>
          Generated {new Date((asset.serviceThreatModel as any).generatedAt).toLocaleDateString()}
        </div>
      )}
    </div>
  );
...
export function AssetDetailsPage() {
  ...
  const loadAsset = async () => {
    ...
    try {
      ...
      setAsset(data);
      if (data.serviceDfd) setGraphView('dfd');
    } catch (e: any) {
  ...
  const loadGraph = async () => {
    ...
    try {
      ...
      const NOISY_TYPES = new Set(['CodeRepository', 'code_repository', 'CodeModule', 'code_module']);
      if (rel?.graph) {
        const filteredNodes = rel.graph.nodes.filter((n) => !NOISY_TYPES.has(n.type));
        const removedIds = new Set(rel.graph.nodes.filter((n) => NOISY_TYPES.has(n.type)).map((n) => n.id));
        setGraph({
          ...rel.graph,
          nodes: filteredNodes,
          edges: rel.graph.edges.filter((e) => !removedIds.has(e.from) && !removedIds.has(e.to)),
        });
      } else if (rel?.nodes?.length) {
        const filteredNodes = rel.nodes.filter((n) => !NOISY_TYPES.has(n.type) && !NOISY_TYPES.has(mapEntityTypeToGraphType(n.type)));
        const removedIds = new Set(rel.nodes.filter((n) => NOISY_TYPES.has(n.type) || NOISY_TYPES.has(mapEntityTypeToGraphType(n.type))).map((n) => n.id));
        setGraph({
          nodes: filteredNodes.map((n) => ({
            id: n.id, type: mapEntityTypeToGraphType(n.type), label: n.name,
            metadata: n.metadata as Record<string, unknown>, link: n.metadata?.link as string | undefined,
          })),
          edges: rel.edges
            .filter((e) => !removedIds.has(e.source) && !removedIds.has(e.target))
            .map((e) => ({
              id: e.id, from: e.source, to: e.target, type: e.type,
              confidence: (e.metadata?.confidence as string) || 'deterministic',
              label: (e.metadata?.label as string) || formatEdgeLabel(e.type),
            })),
          focusNodeId: assetId,
        });
      }
  ...
  const tabBadges: Partial<Record<PanelTabId, number>> = {
    ...
    security: (asset?.serviceThreatModel as any)?.strideThreats?.length ?? (asset?.threatModel as any)?.identifiedThreats?.length ?? 0,
};
