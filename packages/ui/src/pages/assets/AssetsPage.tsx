...
import { FeatureListView } from '../../components/features/FeatureListView';
import { DataStoreListView } from '../../components/data-stores/DataStoreListView';
import { RepositoryDetailPanel } from '../../components/assets/RepositoryDetailPanel';
...
function ScanDialog({ open, loading, repositories, loadingRepos, onStart, onCancel, isDark, hasCloudIntegration, cloudIntegrationName, codeScanAvailable, cloudScanAvailable }: ScanDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      setOpts({
        ...
        scope: codeScanAvailable ? 'all' : 'cloud',
        runType: 'incremental',
      });
  ...
  }, [open, hasCloudIntegration, codeScanAvailable]);
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {(['all', 'code', 'cloud'] as const).map(scope => {
            const disabled = scope === 'cloud' ? !cloudScanAvailable : !codeScanAvailable;
            return (
            <button key={scope} onClick={() => !disabled && setOpts(o => ({ ...o, scope }))} disabled={disabled} style={{
              padding: '6px 14px', borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: 'pointer',
              border: opts.scope === scope ? 'none' : `1px solid ${dk(isDark, T.stone200, D.border)}`,
              background: disabled ? dk(isDark, T.stone100, D.bgSub) : opts.scope === scope ? T.orange : dk(isDark, T.white, D.bgCard),
              color: disabled ? dk(isDark, T.stone300, D.textFaint) : opts.scope === scope ? T.white : dk(isDark, T.stone600, D.textMuted),
            }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '11px 0', borderBottom: `1px solid ${dk(isDark, T.stone100, D.border)}` }}>
        <div style={{ flex: 1, paddingRight: 16 }}>
          ...
          <div style={{ fontSize: 12, color: dk(isDark, T.stone400, D.textMuted), lineHeight: 1.5 }}>
            {hasCloudIntegration
              ? <>Will use <strong>{cloudIntegrationName ?? 'Microsoft Azure'}</strong> to discover and index Azure resources.</>
                    ...
                    {asset.metadata?.repositoryName ? (
                      <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                        ...
                        <span style={{
                          ...
                          {asset.metadata.repositoryName as string}
                        </span>
                      </div>
                    ) : (
                        ...
                        {!!asset.metadata?.responsibility && (
                          <div>
                            ...
                            <div style={{ padding: '8px 12px', background: dk(isDark, T.white, D.bgCard), border: `1px solid ${dk(isDark, T.stone200, D.border)}`, borderRadius: 8, fontSize: 12, color: dk(isDark, T.stone600, D.textMuted), lineHeight: 1.6 }}>
                              {asset.metadata.responsibility as string}
                            </div>
                          </div>
                        )}
                        <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', fontSize: 12, color: dk(isDark, T.stone500, D.textMuted) }}>
                          {!!asset.metadata?.url && <span><b style={{ color: dk(isDark, T.stone600, D.text) }}>URL:</b> {asset.metadata.url as string}</span>}
                          {!!asset.metadata?.cloudProvider && <span><b style={{ color: dk(isDark, T.stone600, D.text) }}>Cloud:</b> {asset.metadata.cloudProvider as string}</span>}
                          {!!asset.metadata?.resourceType && <span><b style={{ color: dk(isDark, T.stone600, D.text) }}>Resource:</b> {asset.metadata.resourceType as string}</span>}
                          {!!asset.metadata?.dataClassification && <span><b style={{ color: dk(isDark, T.stone600, D.text) }}>Classification:</b> {asset.metadata.dataClassification as string}</span>}
                          ...
                          {asset.type === 'azure_identity' && !!asset.metadata?.principalId && (
                            <span>
                              ...
                              <span style={{ fontFamily: 'monospace', fontSize: 11 }}>{asset.metadata.principalId as string}</span>
                            </span>
                          )}
                          {asset.type === 'azure_identity' && !!asset.metadata?.clientId && (
                            <span>
                              <b style={{ color: dk(isDark, T.stone600, D.text) }}>Client ID:</b>{' '}
                              <span style={{ fontFamily: 'monospace', fontSize: 11 }}>{asset.metadata.clientId as string}</span>
                            </span>
                          )}
                          {asset.type === 'azure_identity' && !!asset.metadata?.region && (
                            <span><b style={{ color: dk(isDark, T.stone600, D.text) }}>Region:</b> {asset.metadata.region as string}</span>
                          )}
                        </div>
                        <button
...
export function AssetsPage() {
  ...
  const [activeTab, setActiveTab] = useState<string>(
    searchParams.get('tab') === 'features' ? 'features'
    : searchParams.get('tab') === 'data_stores' ? 'data_stores'
    : 'assets'
  );
  ...
  const { fetchStores: fetchDataStores, stores: dataStores } = useDataStores();
  const { byId: capabilityById, capabilities } = useCapabilities();
  ...
  useEffect(() => {
    ...
    const cloudIntegration = capabilities?.integrations.find(integration => integration.category === 'cloud' && integration.connected);
    setHasCloudIntegration(Boolean(cloudIntegration));
    setCloudIntegrationName(cloudIntegration?.displayName);
  }, [scanDialogOpen, capabilities]);
  ...
  const NAV_DATA_STORES = '__data_stores__';
  const activeView = activeTab === 'features' ? NAV_FEATURES : activeTab === 'data_stores' ? NAV_DATA_STORES : selectedCategory;
  const scanRunning = activeScan?.status === 'queued' || activeScan?.status === 'running';
  const portalCodeScan = capabilityById.get('portalCodeScan');
  const portalCloudScan = capabilityById.get('portalCloudScan');
  const canStartPortalScan = Boolean(portalCodeScan?.available || portalCloudScan?.available);
  const scanDisabledReason = !canStartPortalScan
    ? (portalCodeScan?.reasons.length ? portalCodeScan.reasons.join('. ') : 'Portal scans require LLM, embeddings, and a connected integration.')
    : undefined;
  const navItems = [
    ...categories.map(c => ({ key: c.type, icon: categoryIcons[c.type], label: c.label, count: c.count })),
    { key: NAV_DATA_STORES, icon: <DatabaseOutlined />, label: 'Data Stores', count: dataStores.length || undefined },
  ...
  const handleNavSelect = (key: string) => {
    ...
    } else if (key === NAV_DATA_STORES) {
      setActiveTab('data_stores');
      setSearchParams({ tab: 'data_stores' }, { replace: true });
    } else {
      setActiveTab('assets');
  ...
  return (
    <>
        ...
        <div style={{ flex: 1, minWidth: 0, background: dk(isDark, T.white, D.bg), borderRadius: '0 12px 12px 0', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '14px 24px', borderBottom: `1px solid ${dk(isDark, T.stone100, D.border)}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
            <div>
              <h2 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: dk(isDark, T.stone900, D.text) }}>
                {activeTab === 'features' ? 'Feature Analysis' : activeTab === 'data_stores' ? 'Data Stores' : categories.find(c => c.type === selectedCategory)?.label ?? 'Assets'}
              </h2>
              <p style={{ margin: 0, fontSize: 12, color: dk(isDark, T.stone400, D.textMuted), marginTop: 2 }}>
                {activeTab === 'features'
                  ...
                  : activeTab === 'data_stores'
                    ? 'AI-indexed databases, caches, blob storage, queues & file systems with service & feature links'
                    : selectedRepoId
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <button
                onClick={() => {
                  if (!canStartPortalScan) {
                    notification.info({
                      message: 'Portal scan needs setup',
                      description: scanDisabledReason,
                      duration: 6,
                    });
                    return;
                  }
                ...
                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 16px', borderRadius: 8, border: 'none', background: scanRunning || !canStartPortalScan ? dk(isDark, T.stone200, D.bgCard) : T.orange, color: scanRunning || !canStartPortalScan ? dk(isDark, T.stone400, D.textFaint) : T.white, fontSize: 13, fontWeight: 600, cursor: scanRunning ? 'not-allowed' : !canStartPortalScan ? 'help' : 'pointer' }}
              >
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: selectedRepoId ? 0 : '16px 24px' }}>
            {activeTab === 'features' ? (
              ...
              <DataStoreListView onSelect={(id) => navigate(`/knowledge-base/data-stores/${encodeURIComponent(id)}`)} />
            ) : isEmpty ? (
              <EmptyState onScan={() => {
                if (!canStartPortalScan) {
                  notification.info({
                    message: 'Use local agent indexing first',
                    description: scanDisabledReason,
                    duration: 6,
                  });
                  return;
      ...
      <ScanDialog open={scanDialogOpen} loading={scanLoading} repositories={repositories} loadingRepos={loadingRepos} onStart={handleStartScan} onCancel={() => setScanDialogOpen(false)} isDark={isDark} hasCloudIntegration={hasCloudIntegration} cloudIntegrationName={cloudIntegrationName} codeScanAvailable={Boolean(portalCodeScan?.available)} cloudScanAvailable={Boolean(portalCloudScan?.available)} />
    </>
  );
}
