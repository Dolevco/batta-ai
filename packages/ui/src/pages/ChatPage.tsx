...
import { ChatEntityTable } from '../components/ChatEntityTable';
import type { BusinessFeature } from '../types';
import type { TableProjection } from '../components/ChatEntityTable';
...
function BattaIcon({ size = 20 }: { size?: number }) {
  return (
    <img
      src="/images/batta.svg"
      alt="Batta"
      style={{
        width: size,
        height: size,
...
export function ChatPage() {
  ...
  return (
    <>
      <style>{`
        @keyframes batta-blink { 0%,100%{opacity:1} 50%{opacity:0.2} }
        @keyframes batta-pulse {
          0%,100% { opacity:1; transform:scale(1); }
          50%      { opacity:0.6; transform:scale(1.15); }
        }

        .batta-msg-enter { animation: batta-msg-in 0.25s ease-out both; }
        @keyframes batta-msg-in {
          from { opacity: 0; transform: translateY(6px); }
          to   { opacity: 1; transform: translateY(0); }
        }

        .batta-input-wrap { transition: box-shadow 0.15s, border-color 0.15s; }
        .batta-input-wrap:focus-within {
          border-color: ${T.orange} !important;
          box-shadow: 0 0 0 3px rgba(249,115,22,0.1) !important;
        }

        .batta-chip { transition: border-color 0.15s, background 0.15s, color 0.15s; }
        .batta-chip:hover {
          border-color: ${T.orange} !important;
        ...
        .batta-messages::-webkit-scrollbar { width: 5px; }
        .batta-messages::-webkit-scrollbar-track { background: transparent; }
        .batta-messages::-webkit-scrollbar-thumb {
        ...
        >
          <div style={{ maxWidth: MAX_W, margin: '0 auto', padding: '48px 24px 0' }}>
            ...
            {(loading || streamingContent || chainOfThought) && (
              <div className="batta-msg-enter" style={{
                display: 'flex', gap: 16, alignItems: 'flex-start',
                marginBottom: 32,
              }}>
                <div style={{ flexShrink: 0, paddingTop: 2 }}>
                  ...
                  {streamingContent && (
                    <div style={{
                      ...
                      <span style={{
                        ...
                        animation: 'batta-blink 0.9s infinite',
                      }} />
                    </div>
                  )}
