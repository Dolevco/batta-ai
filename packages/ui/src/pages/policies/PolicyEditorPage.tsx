...
import type { PolicyTemplate, PolicyTemplateType, SecurityTask, JiraActionItemsConfig } from '../../types';
...
const TYPE_ICONS: Record<PolicyTemplateType, React.ReactNode> = {
  ...
  work_item_review: <IssuesCloseOutlined />,
};
...
const FRAMEWORK_SECTIONS: Record<PolicyTemplateType, FrameworkSection[]> = {
  ...
  work_item_review: [
    {
      icon: <QuestionCircleOutlined />, label: 'Questionnaire',
      description: 'Security questions answered autonomously by the agent when a Jira work item is reviewed.',
      tag: 'Editable', color: T.teal, bg: T.tealLight, border: T.tealBorder, darkBg: D.tealLight, darkBorder: D.tealBorder,
      details: ['Data handling & PII', 'New endpoints exposed', 'Authentication changes', 'External integrations', 'Data migrations', 'Infrastructure changes'],
    },
    {
      icon: <ThunderboltOutlined />, label: 'Conditional Task Rules',
      description: 'Security tasks automatically created when the agent answers "yes" to risk questions.',
      tag: 'Editable', color: T.amber, bg: T.amberLight, border: T.amberBorder, darkBg: D.amberLight, darkBorder: 'rgba(217,119,6,0.3)',
      details: ['Triggered by agent yes-answers', 'Severity-tiered (critical→low)', 'Principle-tagged tasks', 'Per-question task sets'],
    },
    {
      icon: <AuditOutlined />, label: 'Baseline Controls',
      description: 'Mandatory tasks always included for every Jira work item review.',
      tag: 'Editable', color: T.green, bg: T.greenLight, border: T.greenBorder, darkBg: D.greenLight, darkBorder: 'rgba(22,163,74,0.3)',
      details: ['Work item traceability', 'Security review link in Jira', 'Audit trail creation'],
    },
    {
      icon: <IssuesCloseOutlined />, label: 'Jira Action Items',
      description: 'Auto-export security tasks back to Jira issues on attestation.',
      tag: 'Automation', color: '#2563EB', bg: '#EFF6FF', border: '#BFDBFE', darkBg: '#1e2d45', darkBorder: '#2d4a6e',
      details: ['Configurable severity threshold', 'Target project key', 'Issue type mapping', 'Priority map per severity', 'Auto-create on attestation'],
    },
  ],
};
...
const DEFAULT_JIRA_CONFIG: JiraActionItemsConfig = {
  autoCreate: false,
...
function JiraAutomationTab({ policy, isDark, mutate }: {
  policy: PolicyTemplate;
  isDark: boolean;
  mutate: (fn: (p: PolicyTemplate) => PolicyTemplate) => void;
}) {
  const jira = policy.jiraActionItems ?? DEFAULT_JIRA_CONFIG;

  function update(patch: Partial<JiraActionItemsConfig>) {
    mutate(p => ({
      ...p,
      jiraActionItems: { ...(p.jiraActionItems ?? DEFAULT_JIRA_CONFIG), ...patch },
    }));
  }

  function updatePriorityMap(key: keyof JiraActionItemsConfig['priorityMap'], val: string) {
    mutate(p => {
      const current = p.jiraActionItems ?? DEFAULT_JIRA_CONFIG;
      return {
        ...p,
        jiraActionItems: {
          ...current,
          priorityMap: { ...current.priorityMap, [key]: val },
        },
      };
    });
  }
  ...
  const jiraBlue = '#2563EB';
  const jiraBlueBg = dk(isDark, '#EFF6FF', '#1e2d45');
  const jiraBlueBorder = dk(isDark, '#BFDBFE', '#2d4a6e');

  const projectKeyValid = !jira.targetProjectKey || /^[A-Z][A-Z0-9]*$/.test(jira.targetProjectKey);
      ...
      <div style={{ maxWidth: 560, display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ background: dk(isDark, T.white, D.bgCard), border: `1px solid ${dk(isDark, T.stone200, D.borderSub)}`, borderRadius: 14, padding: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, color: dk(isDark, T.stone700, D.text), marginBottom: 4 }}>
            ...
            <Switch
              checked={jira.autoCreate}
              onChange={val => update({ autoCreate: val })}
            />
          </div>
        </div>
        ...
        {jira.autoCreate && (
          <>
            ...
            <div style={{ background: dk(isDark, T.white, D.bgCard), border: `1px solid ${dk(isDark, T.stone200, D.borderSub)}`, borderRadius: 14, padding: 20 }}>
              ...
              <input
                value={jira.targetProjectKey}
                onChange={e => update({ targetProjectKey: e.target.value.toUpperCase() })}
                placeholder="SEC"
                maxLength={20}
                style={{ ...fieldStyle, fontFamily: 'monospace', borderColor: !projectKeyValid ? T.red : undefined }}
              />
              {!projectKeyValid && (
                <div style={{ fontSize: 11, color: T.red, marginTop: 4 }}>
                  Must match pattern: capital letter followed by capital letters or digits (e.g. SEC, ENG2)
                </div>
              )}
              {projectKeyValid && (
                <div style={{ fontSize: 11, color: dk(isDark, T.stone400, D.textFaint), marginTop: 4 }}>
                  e.g. SEC, ENG — capital letters and digits only
                </div>
              )}
            </div>
            ...
            <div style={{ background: dk(isDark, T.white, D.bgCard), border: `1px solid ${dk(isDark, T.stone200, D.borderSub)}`, borderRadius: 14, padding: 20 }}>
              ...
              <select
                value={jira.severityThreshold}
                onChange={e => update({ severityThreshold: e.target.value as JiraActionItemsConfig['severityThreshold'] })}
                style={fieldStyle}
              >
                ...
                {(['critical', 'high', 'medium', 'low'] as const).map(sev => {
                  ...
                  return (
                    <div key={sev} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <span style={{ fontSize: 12, color: dk(isDark, T.stone400, D.textFaint), flexShrink: 0 }}>→</span>
                      <input
                        value={jira.priorityMap[sev]}
                        onChange={e => updatePriorityMap(sev, e.target.value)}
                        placeholder={sev === 'critical' ? 'Highest' : sev === 'high' ? 'High' : sev === 'medium' ? 'Medium' : 'Low'}
                        maxLength={50}
        ...
        <div style={{
          ...
          background: jiraBlueBg,
          border: `1px solid ${jiraBlueBorder}`,
          fontSize: 12, color: jiraBlue, lineHeight: 1.5,
        }}>
          <InfoCircleOutlined style={{ marginRight: 8 }} />
          When enabled, Jira issues are created automatically after agent analysis completes —
          no attestation step is required for work item reviews.
          Manual export is always available from the review detail page.
        </div>
      </div>
    </div>
  );
...
type TabId = 'overview' | 'questionnaire' | 'tasks' | 'settings' | 'jira-automation';

const BASE_TAB_DEFS: Array<{ id: TabId; icon: React.ReactNode; label: string; sublabel: string; onlyFor?: PolicyTemplateType[] }> = [
  ...
  { id: 'jira-automation', icon: <IssuesCloseOutlined />,   label: 'Jira Automation',   sublabel: 'Auto-export config', onlyFor: ['work_item_review'] },
];
...
function LeftNav({ active, onChange, accentColor, isDark, policy }: {
  ...
  const TAB_DEFS = BASE_TAB_DEFS.filter(t => !t.onlyFor || t.onlyFor.includes(policy.type));
  ...
  return (
    <div style={{ maxWidth: 860, padding: '28px 32px' }}>
      ...
      <div style={{
        ...
        {policy.type === 'work_item_review' && (
          <> The <strong>Jira Automation</strong> tab configures auto-export of security action items back to Jira.</>
        )}
      </div>
    </div>
  );
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{ fontSize: 11, fontWeight: 700, color: accentColor }}>
            {localPolicy.type === 'security_review' ? 'Security' : localPolicy.type === 'responsible_ai' ? 'Responsible AI' : localPolicy.type === 'privacy' ? 'Privacy' : 'Work Item Review'}
          </span>
        </div>
