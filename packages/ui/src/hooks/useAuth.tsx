...
import { getAuthConfig, cacheAuthProvider } from '../services/auth/authConfig';
import { createOidcProvider } from '../services/auth/providers/oidcProvider';
import type { AuthProvider } from '../services/auth/providers/msalProvider';

// Loaded on demand so @azure/msal-browser is only bundled when MSAL is configured.
const loadMsalProvider = () =>
  import('../services/auth/providers/msalProvider').then(m => m.createMsalProvider);
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      try {
        ...
        // Create the appropriate provider — MSAL is loaded lazily to avoid
        // bundling @azure/msal-browser when a different auth provider is used.
        if (config.provider === 'msal' && config.msal) {
          const createMsalProvider = await loadMsalProvider();
  ...
  if (AUTH_DISABLED) {
    return <AuthContext.Provider value={NO_AUTH_VALUE}>{children}</AuthContext.Provider>;
  }
